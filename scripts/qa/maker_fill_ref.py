...
RULE_DECISIONS = [
    ...
    "at or before insertion time, used as-is. An order that improves the "
    "touch (S2 inside-quote) has queue-ahead = 0 by construction. No "
    "attempt is made to purify a pre-existing own footprint baked into a "
    "reused tape's displayed sizes -- this is why this simulator's "
    "numbers on a reused tape can differ from that tape's original "
    "packet's sealed values (expected; see docs/QA/answers_sealed_maker4.json).",
    ...
    "Forced exit at the cap: exit_ts = entry_ts + cap_s exactly; "
    "exit_price = the touch price of the exit's side as last known at "
    "that instant (no additional slippage, per the rule text).",
...
def simulate(ticker_df: pd.DataFrame, exec_df: pd.DataFrame, strategy: str = "S1",
        ...
        for side in ("bid", "ask"):
            ...
            if pos is not None and now >= pos.entry_ts + pd.Timedelta(seconds=cap_s):
                ...
                rows_out.append(dict(direction=("long" if side == "bid" else "short"),
                                      ...
                                      exit_ts=deadline, exit_price=touch[exit_side], forced=True,
    ...
    def refresh(side):
        ...
        if cur is not None and cur.role == role and cur.price == price:
            return
        qa = 0.0 if price != touch[side] else (disp[side] or 0.0)
