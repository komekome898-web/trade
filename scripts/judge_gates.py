...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    # **必須にする(2026-09-13、10 本目の監査)。**
    # 9 本目の指摘を受けて `--no-audit-gate` という旗を消したが、**挙動は一字も変わっていなかった**:
    # `--unit` を省けば関門は呼ばれず、標準エラーに 1 行出すだけで判定結果が出ていた。
    # 「明示的に付ける旗」から「何もしなければそうなる既定動作」に変わり、**発見されにくくなった**。
    # → 単位名を必須にして、**関門を通らない経路そのものを無くす。**
    ap.add_argument("--unit", required=True,
                    help="研究単位の名前(必須)。docs/AUDITOR/ACTION_LOG.md に "
