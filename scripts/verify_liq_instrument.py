...
BG_NOISE_BP = 0.5      # 背景の価格点に乗せるノイズ(bp)。測定ノイズ NOISE_STD_BP=4.0 の 1/8。
                       # 意図は「バーの間は静かで、カスケードだけが動く」状況を作ること。
                       # **初版の注記は「この値で n_tie が決まるので、変えたら結果が変わる」と
                       # 書いていた。振ったら誤りだった**(`evidence_2026-09-14/13`):
                       # **0.25 / 0.5 / 1.0 / 2.0 / 4.0 / 8.0 のどれでも n_tie は 41 で一定。**
                       # 14 本目の監査が「一度も振っていない値」「タイが出るように小さく取った
                       # = 合格を出すために置いた値の形」(A-12)と指摘したので振った。
                       # **指摘は当たっていた**(振っていなかったのは事実)**が、恐れられた
                       # 「この値が結果を決める」ではなく「理由の書き方が誤り」だった。**
                       # n_tie を決めているのはカスケード長とバー幅の関係で、**その機構は未確認**
                       # (1〜10 秒のカスケードは 60 秒バーでタイ 0、300 秒バーで 100% と、
                       #  リードが想定した向きと違う。同 log)。
...
def known_direction_estimator(bundle, tick_bp: float, horizon: int) -> float:
    ...
    **ただし読み出しは渡された系列のままなので、系列が粗ければその劣化は被る。**
    (初版はここに「つまり『これ以上うまくはできない』上限である」「これが仕込み値を
     取り出せるなら、取り出せない方針は測定器側の問題である」と書いていた。
     **撤回した文をそのまま同じ説明の中に残していたので消した。**14 本目の監査の [止める] 2。
     **報告書からコードへ、同じ型が移っただけだった。**)
    """
