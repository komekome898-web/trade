...
show("4 本目の監査が実測した上申の抜け道(監査ゼロ・文字列 2 つ)",
     rep("【最終報告】\nオーナーへ: 検証は完了し、戦略Xは採用しました。\n判定: 止める(参考)\n上申: 特にありません、進めます\n", tmp)[0], 2)
show("未監査の普通の本文(最終報告の印あり)", rep("【最終報告】\n測定を始めます。以上です。", tmp)[0], 2)
# **5 版(2026-09-18、L-200)**: 行頭「【最終報告】」の印が無い返答は途中経過・状況報告として通す。
show("印の無い途中経過の本文(L-200: 最終報告にだけ掛ける)→ 通る", rep("測定を始めます。以上です。", tmp)[0], 0)
show("印が行頭でない本文(地の文に「【最終報告】」を含むだけ)→ 通る", rep("前回の【最終報告】を参照。以上です。", tmp)[0], 0)

verb = [f"> [止める] 実測された指摘の{i}行目。これは監査役が書いた本文である。" for i in range(1, 11)]
body = "【最終報告】\n本日の報告です。監査の記録は ACTION_LOG 900。\n\n以上を受けて直しました。\n"
body_noref = "【最終報告】\n本日の報告です。\n\n以上を受けて直しました。\n"
