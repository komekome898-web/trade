...
出所は (削除済み文書)・`99_master.md`(§5.2 の分類: 元データ消失/ログに必要列が無い
/データ不足)、(削除済み文書) §0、(削除済み文書)。台帳自体は追記専用(行の削除・書き換えは
...
| 13 | maker の逆選択(6〜9bps 主張、第 i・j 報)の元テープ | 第 i・j 報作成時点(日付不明) | 元テープが消失し、E2 での再検証(約定後 5 秒 +0.88bps・30 秒 +0.93bps、主張の 7〜10 倍過大)は別テープでの近似にとどまる((削除済み文書) L5 行、AR-19 の注記「第 x 報のテープは消失」) | 不可(元テープは既に無く、法則の大きさの直接再検証はできない。閉鎖の結論自体は E2 で別テープにより再現) | 2026-09-05 |
