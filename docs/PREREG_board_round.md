...
**付記3(2026-09-04、走査母集団の訂正。付記2 の絶対水準・全時刻走査と、一時的に書いた「対照が静か過ぎた」
診断は撤回)**: オーナー指摘のとおり、初発は定義上「直前30分にバーストなし」の状態でしか起きず、§3 の対照も
同じ状態から採っている。したがってスクリーニング AUC(板更新率 0.81・スプレッド 0.76)は「**静かな状態の中で、
1〜3分後に嵐が来る時刻を見分けられるか**」への正当な答えであり、付記2 の走査が全時刻(嵐の最中・直後を含む)
の分布で線を引いたことが誤りだった。全時刻無作為対照の AUC(0.62〜0.64)は別の問いへの答えで、判定には使わない。

**正式な参照記録 = 状態内走査**(`tp_operating_curve.py` PRIMARY 節、状態 = 直前30分にバースト・ビンなし、
全時間の 68%、状態内の初発基礎率 2.3%、命中 = 180秒以内の初発):

| 特徴量(状態内分位) | 警報/日 | 適合率 | 再現率 | 基礎率比 |
|---|---|---|---|---|
| 2特徴合成 p90 / p95 / p99 | 128 / 70 / 16 | 8.3% / 11.7% / 17.1% | 54% / 41% / 12% | 3.6 / 5.1 / 7.4 |
| 板更新率 p95 / p97.5 | 71 / 39 | 11.1% / 14.4% | 40% / 29% | 4.8 / 6.2 |
| 平均スプレッド p95 | 70 | 6.4% | 28% | 2.8 |
| 実現変位(ベンチマーク)p95 / p97.5 / p99 | 108 / 60 / 27 | 10.1% / 12.4% / 19.5% | 50% / 28% / 13% | 4.3 / 5.3 / 8.4 |

読み: 板の2特徴は、1日約70回の警報(各3分の警戒 ≈ 3.5時間/日)で初発の 41% を基礎率の 5 倍の確度で捉え、
同程度の警報回数の価格変位ベンチマーク(60/日: 12.4%・28%)より再現率が高い(F1 0.18 vs 0.17)。
「静かな状態の中の前兆」は本物で、実用の入口にある。

**第2段の運用点と有用性バー(今固定、第2段データを見る前)**: 状態 = 直前30分にバーストなし。特徴 = 2特徴合成
(状態内パーセンタイル順位の平均)。**運用点 = 本系列の状態内 p95 に相当する閾値 0.771**(第2段では固定値として
適用し、分位を取り直さない)。バー: 第2段データで (i) 事前登録の AUC バー、(ii) 運用点で **適合率 ≥ 基礎率の 3 倍
かつ 再現率 ≥ 30%**、(iii) 警報/日を揃えた実現変位ベンチマークに対し **F1 ≥ ベンチマーク F1**。
(ii)(iii) のどちらかが落ちれば「既知の規模法則の再表現」として閉鎖。通過時の用途は方向不問の 1〜3 分先の
荒れ予告(maker/ストップの回避側、S12 の継続判断側)に限り、モジュール設計は別 PREREG。
