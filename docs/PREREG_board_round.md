...
読み: スクリーニングの AUC(時刻帯を揃えた対照との相対順位)は本物だが、**絶対水準の閾値では基礎率を下回り、
ベンチマークの規模持続に劣る**。板更新率は記録の配信間隔で飽和しており(5秒あたり中央値 36、上限 ≈43)、
動的範囲が狭い。第2段では事前登録のバー(AUC)に加え、**有用性バー**を今固定する:
「警報 ≤ 20 回/日の閾値で、適合率 ≥ 基礎率の 3 倍 かつ 再現率 ≥ 30%、かつ同じ警報回数でベンチマークを適合率で
上回る」。AUC バーのみ通過で有用性バーが落ちた場合は「既知の規模法則の再表現」として閉鎖する。
