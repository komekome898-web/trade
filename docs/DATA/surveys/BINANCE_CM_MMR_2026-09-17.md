...
### 判定
**【追記 2 で改めた: 式の画像 3 枚を取得して目視転記 = 一次資料で確認。以下は初稿の判定】** **一次資料に到達できず(方法: WebFetch、コード: 200相当で到達自体は成功。ただし清算価格の式本体はページ内で画像として提供されており、alt テキストが無いためテキストとして原文引用ができない)。**
...
| BTCUSD_PERP の完全な段階表(全段階) | 一次資料で確認(追記。公開 JSON [15][19]、2026-05-11 更新の表。2023〜2024 年当時の表かは不明。初稿は「到達できず(202 / 0 バイト・SPA)」) | [15][19] |
