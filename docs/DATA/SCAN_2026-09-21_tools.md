...
| `GFT Backtest Software` | 未確認(公開の文書に記事が 1 件も無い) | 未確認 | 未確認 | **登録の内側**(4 経路は全部塞がったが、それは本体が登録の内側にあるためで、当方は登録しない。20 回目に直した = 19 回目のリードの検収 §4(5)) |
...
1. `lo2cin4/lo2cin4bt`(44 番) — 状態: **浅い(足の契約の原典から粒度を確定)。**印は `区分1-足`、および `区分4 へ`(`区分1-ベクトル化` は 20 回目に外した = 19 回目のリードの検収 §4(6))。
...
## 区分1 — 20 回目の実行(2026-09-22)

委任文: `docs/DATA/delegations/20260922_tools_survey_prompt.md@ce0012c95154`。生ログ: `docs/DATA/probes/20260922_tools_1_run20.log`。
19 回目のリードの検収(`docs/AUDITOR/VERDICTS/2026-09-22_tools_scan_cat1_run19.md`)と、その §7 の起動の指定に従う。
起動の指定の逐語「**最優先: 候補の一覧 120 件の重複を機械で洗い出し、一覧にして返す。**」「**残りの本体: `区分1-イベント駆動` の 9 件。**」
「**新しい検索計画は打たないでください。候補は 1 件も増やさないでください。**」「**深掘り(構築・導入・最小実行)は要りません。**」のとおりに進めた。

**3 番と 73 番は同じ登録である。以後 73 番を正とする**(19 回目のリードの検収 §4(1) の逐語「**20 回目の節に「3 番と 73 番は同じ登録。以後 73 番を正とする」と 1 行書いて、そこで繋ぐこと。**」)。
1 回目・2 回目の古い節は書き換えていない(その時点の測定の記録であるため)。

**既存の節への書き換えは、起動の指定が許した 2 箇所ちょうどである。**`git diff` の削除行は 2 行で、内訳は
(a) 候補 44 の印から `区分1-ベクトル化` を外した行(19 回目の検収 §4(6))と、
(b) ツールの表の候補 49 の「到達」の欄を「登録の内側」の趣旨に直した行(同 §4(5))だけである。
**知見 6(4 経路を打って何が返ったか)は測定の記録なので消していない。**候補 74 の `区分1-ベクトル化` はそのままにしてある。

**原典の字形について**: この回に逐語で引いた原典の行のうち、**非 ASCII の文字が入っているのは 1 つだけで、候補 59 の「- Tick级别回测」である**(中国語。README の全体が中国語で書かれている)。
残りの引用は全部 ASCII の半角文字だけで、非分割ハイフン(`‑` U+2011)などの字形違いは入っていない。
**ただし候補 52 と候補 63 の引用には、原文の二重空白がそのまま入っている**(該当の行に「その旨」を添えてある)。
**原典の在処は、逐語を取る前に必ず追跡ファイルの一覧(`ungh.cc` の `files`)で確かめてから取った**(19 回目の検収 §7(6) の申し送り)。
この手順で、39 番の一覧に `README.md` のほかに `README_ja.md` と `README_zh.md` が在ることと、
候補 52 の README の綴りが小文字の `readme.md` であることと、候補 41 の既定枝が `v4.1-alpha` であることを、当てずっぽうの前に確かめている。
§8 の `tools_inventory.py` の全文は、10 回目の検収 §4-3 の判断により区分 1 の 1 回目の節を参照して貼っていない。

### 検索計画

**この回は検索計画を打っていない。**起動の指定の逐語「**新しい検索計画は打たないでください。候補は 1 件も増やさないでください。**」による。
X の経路も同じ理由で打っていない。**新しい候補は 0 件で、候補の一覧は 120 番のままである。**
39 番の一覧(`awesome-systematic-trading` の README)を取り直したが、これは新しい検索ではなく、
51 番から 89 番の**所有者と登録名を突き合わせの鍵として取り出すため**の既存の出典の確認である。

### 出典

| # | 出典 | 取得日 | 使った先 |
|---|---|---|---|
| 1 | `https://ungh.cc/repos/paperswithbacktest/awesome-systematic-trading/files/main` | 2026-09-22 | 39 番の一覧の追跡ファイルの在処(`README.md` / `README_ja.md` / `README_zh.md`)の確認 |
| 2 | `https://raw.githubusercontent.com/paperswithbacktest/awesome-systematic-trading/main/README.md` | 2026-09-22 | 51 番から 89 番の `所有者/登録名` の抽出(重複の突き合わせの鍵) |
| 3 | `https://ungh.cc/repos/evan-kolberg/prediction-market-backtesting` と `.../files/v4.1-alpha` と `.../README.md` | 2026-09-22 | 候補 41 の星・作成日・既定枝・追跡ファイル・機能の逐語 |
| 4 | `https://ungh.cc/repos/QuantConnect/Lean`・`.../files/master`・`https://raw.githubusercontent.com/QuantConnect/Lean/master/readme.md` | 2026-09-22 | 候補 52 の星・作成日・イベント駆動の逐語 |
| 5 | `https://raw.githubusercontent.com/QuantConnect/Lean/master/Common/Orders/Fills/FillModel.cs` | 2026-09-22 | 候補 52 の粒度(`Tick` / `TradeBar` / `QuoteBar` の分岐) |
| 6 | `https://ungh.cc/repos/nautechsystems/nautilus_trader`・`.../files/develop`・`.../README.md` | 2026-09-22 | 候補 58 のイベント駆動と検証の入力の逐語 |
| 7 | `https://ungh.cc/repos/barter-rs/barter-rs`・`.../files/main`・`.../README.md` | 2026-09-22 | 候補 61 のイベント駆動の逐語と、粒度の語の不在 |
| 8 | `https://ungh.cc/repos/quarkfin/qf-lib`・`.../files/master`・`.../README.md` | 2026-09-22 | 候補 62 のイベント駆動と粒度の逐語 |
| 9 | `https://ungh.cc/repos/rburkholder/trade-frame`・`.../files/master`・`.../README.md` | 2026-09-22 | 候補 63 の 18 回目の逐語の再確認 |
| 10 | `https://ungh.cc/repos/AsyncAlgoTrading/aat`・`.../files/main`・`.../README.md` | 2026-09-22 | 候補 65 のイベント駆動と板の実装の逐語 |
| 11 | `https://raw.githubusercontent.com/AsyncAlgoTrading/aat/main/aat/core/order_book/price_level/price_level.py` | 2026-09-22 | 候補 65 の価格帯ごとの待ち行列の実装 |
| 12 | `https://ungh.cc/repos/letianzj/quanttrader`・`.../files/master`・`.../README.md` | 2026-09-22 | 候補 68 のイベント駆動の逐語と、粒度の語の不在 |
| 13 | `https://ungh.cc/repos/gobacktest/gobacktest`・`.../files/main`・`.../README.md` | 2026-09-22 | 候補 69 のイベント駆動と足の逐語 |
| 14 | `https://ungh.cc/repos/pegasusTrader/PandoraTrader`・`.../files/master`・`.../README.md` | 2026-09-22 | 候補 59 の粒度の逐語 |
| 15 | `https://ungh.cc/repos/Oddpool/PredictionMarketBench`・`.../files/main`・`.../README.md` | 2026-09-22 | 候補 90 の粒度と待ち行列の逐語 |

### 知見

| # | 知見 | 印 | 根拠 |
|---|---|---|---|
| 1 | **現行の候補 1 番から 120 番の中に、同じ登録を 2 つ以上の番号で数えている組は 1 つも無かった。**突き合わせは `所有者/登録名`(GitHub)と配布物の名前(PyPI)で行い、鍵は 101 件について取れた。鍵の出所の内訳は、この回に取り直した 39 番の一覧の被リンクが 39 件、報告の本文に既に書かれている所有者/登録名が 41 件、既存の生ログから機械抽出した URL が 21 件である。**記憶では埋めていない** | 実測 | `20260922_tools_1_run20.log:472`・`:473`・`:523`・`:524` |
| 2 | **重複は、現行の番号どうしではなく、1 回目・2 回目の節の独自の番号と現行の番号の間に在った。**同じ登録の組は 25 件で、うち 21 件は現行の 1 番から 23 番への意図された番号の振り直しである(1 回目・2 回目の節は現行とは別の番号体系を使っている)。**残る 4 件が、1 回目・2 回目に到達済みだった登録を、39 番の一覧から「未着手」の新しい候補として立て直したものである** | 実測 | `20260922_tools_1_run20.log:474`〜`:500` |
| 3 | **その 4 件の内訳。**旧 3 番 `vectorbt / vectorbt.pro` = **73 番**(19 回目にリードが見つけた 1 件)/ 旧 2 番 `NautilusTrader` = **58 番** `nautilus_trader`(鍵 `nautechsystems/nautilus_trader`)/ 旧 4 番 `freqtrade` = **75 番** `Freqtrade`(鍵 `freqtrade/freqtrade`)/ 旧 5 番 `Backtesting.py` = **55 番** `backtesting.py`(鍵 `kernc/backtesting.py`)。**19 回目に見つかっていなかった新しい重複は 3 件である** | 実測 | `20260922_tools_1_run20.log:476`・`:477`・`:478`・`:479` |
| 4 | **その 3 件は、1 回目・2 回目の節では「未着手」ではなく到達済みだった。**2 回目の節の逐語は、55 番について「`Backtesting.py`(`backtesting`、kernc) — 実測 — 軽量バックテスト、AGPL-3.0。**指値の1往復に成功**(最小実行)」であり、75 番について「隔離venvにfreqtrade導入(ccxt 4.5.82同梱)。`freqtrade list-exchanges -a`を実測実行」である。58 番は 1 回目の節で「NautilusTrader(nautechsystems) — 一次資料 — Rust コアの event-drive…」と書かれている。**現行の一覧ではこの 3 件が「未着手」のままになっており、到達の記録が別の節に取り残されている** | 一次資料 | `docs/DATA/SCAN_2026-09-21_tools.md` の 1 回目の節(候補 2)と 2 回目の節(候補 5・912 行) |
| 5 | **逆に、1 回目・2 回目の節に在って、現行の 1 番から 120 番に対応する番号が 1 つも無い登録が 4 件ある。**`mhallsmoore/qstrader`(旧 7 番。2 回目に最小実行まで打っている)/ `gbeced/pyalgotrade`(旧 9 番。2 回目に「**指値の1往復に成功**」と書かれている)/ `carlos8f/zenbot`(旧 10 番。現行 13 番はその分岐 `DeviaVir/zenbot` で、登録としては別物)/ `ccxt/ccxt`(旧 17 番)。**これは重複の逆で、候補の一覧から落ちた側である。番号を立て直すかはリードが決めること** | 実測 | `20260922_tools_1_run20.log:501`〜`:506` |
| 6 | **突き合わせの鍵が取れなかった候補は 11 件である。**40・45・46・48・49・50・107・108・109・117・118 で、いずれも GitHub の登録でも PyPI の配布物でもない(登録の要る外部の場・事業者の頁・論文・記事)。**この 11 件については、重複が無いと機械では言えない。**場の URL で突き合わせようとしたが、そもそも公式の場の URL が確定していない候補(40・48・107・117・118)が含まれる | 実測 | `20260922_tools_1_run20.log:507`〜`:519` |
| 7 | **登録名だけが同じで所有者が違う組が 2 つある。重複ではない。**39 番 `paperswithbacktest/awesome-systematic-trading` と 110 番 `wangzhe3224/awesome-systematic-trading`、97 番 `kahan15/limit-order-book-simulator` と 102 番 `3yit/limit-order-book-simulator` である。**名前だけで突き合わせると誤って束ねる組なので、鍵は所有者込みで取っている** | 実測 | `20260922_tools_1_run20.log:520`〜`:522` |
| 8 | **候補 58 `nautilus_trader` は、区分 1 の 6 要素のうち少なくとも 4 つを同時に持つ。**README の逐語は「**Backtesting**: Multiple venues, instruments, and strategies simultaneously using historical quote tick, trade tick, bar, order book, and custom data with nanosecond resolution.」で、登録情報の説明の逐語は「Production-grade Rust-native trading engine with deterministic event-driven architecture」である。**イベント駆動・ティック・足・板のすべてを検証の入力として名乗っている。**星は 29265、作成は 2018-06-25、既定枝は `develop` | 一次資料 | `https://raw.githubusercontent.com/nautechsystems/nautilus_trader/develop/README.md`(2026-09-22 取得)の 60 行、`20260922_tools_1_run20.log:125`・`:113` |
| 9 | **候補 58 は、研究の側がベクトル化で本番がイベント駆動という分断を、自分の射程として名指ししている。**README の逐語は「Trading strategy research is often conducted in Python using vectorized approaches, while production trading systems are implemented separately using event-driven architectures in」である。**ただし板の待ち行列の位置を模擬するかは README に書かれていない**(この回は原典の実装まで降りていない) | 一次資料 | 同上の 74・75 行、`20260922_tools_1_run20.log:126`・`:127` |
| 10 | **候補 52 `QuantConnect/Lean` はイベント駆動を自分で名乗り、約定の模型が足とティックの両方で分岐する。**README の逐語は「LEAN is an event-driven, professional-caliber algorithmic trading  platform built with a passion for elegant engineering and deep quant  concept modeling.」(原文の二重空白のまま)。約定の模型 `Common/Orders/Fills/FillModel.cs` は `subscribedTypes.Contains(typeof(Tick))`・`subscribedTypes.Contains(typeof(TradeBar))`・`subscribedTypes.Contains(typeof(QuoteBar))` で分岐しており、**足とティックの両方を約定の判定に使う。**星は 21721、作成は 2014-11-28 | 一次資料 | `https://raw.githubusercontent.com/QuantConnect/Lean/master/readme.md`(2026-09-22 取得)の 22 行、`https://raw.githubusercontent.com/QuantConnect/Lean/master/Common/Orders/Fills/FillModel.cs`(同)の 625・637・899・919 行、`20260922_tools_1_run20.log:108`・`:405`・`:99` |
| 11 | **候補 65 `aat` は板の待ち行列を持っている。**README の逐語は「`aat` also provides a complete limit-order book implementation, including flags like `fill-or-kill` and `all-or-nothing`, which is used to power the synthetic testing exchange.」。実装 `aat/core/order_book/price_level/price_level.py` の `add` は、逐語のコメント「# append order to deque」の下で `self._orders.append(order)` としており、**価格帯ごとに注文を到着順の列に積む。**星は 835、作成は 2019-05-20 | 一次資料 | `https://raw.githubusercontent.com/AsyncAlgoTrading/aat/main/README.md`(2026-09-22 取得)の 50 行、`.../aat/core/order_book/price_level/price_level.py`(同)の 45・46・57 行、`20260922_tools_1_run20.log:178`・`:383`・`:384`・`:395`・`:161` |
| 12 | **候補 90 `Oddpool/PredictionMarketBench` は、待ち行列の位置を明示的に模擬すると書いている。**README の逐語は「Orders can rest in the book and receive fills when historical trades match your price. Queue position is simulated based on displayed size and pro-rata fill allocation.」で、入力の逐語は「**Input**: Recorded orderbook snapshots and trade prints from Kalshi」である。**当方の参照実装 `scripts/qa/maker_fill_ref.py` は表示サイズの後ろに並ぶ FIFO(委任文 §8 のログ A-4)で、こちらは pro-rata の配分である。配分の規則が違う。**星は 27、作成は 2026-01-07 | 一次資料 | `https://raw.githubusercontent.com/Oddpool/PredictionMarketBench/main/README.md`(2026-09-22 取得)の 9・134 行、`20260922_tools_1_run20.log:459`・`:469`・`:445`・`:609`・`:610` |
| 13 | **候補 59 `PandoraTrader` の粒度は原典の 1 行で決まった。**README の逐語は「- Tick级别回测」(中国語。「ティック水準の検証」)である。**README は中国語のみで、追跡ファイルの根に `LICENSE` も `COPYING` も無い**(根に在るのは `README.md` だけ)。星は 1472、作成は 2019-01-03 | 一次資料 | `https://raw.githubusercontent.com/pegasusTrader/PandoraTrader/master/README.md`(2026-09-22 取得)の 34 行、`20260922_tools_1_run20.log:444`・`:429`・`:607`・`:608` |
| 14 | **候補 62 `qf-lib` はイベント駆動で、刻むのは日である。**README の逐語は「The Backtester uses an **event-driven architecture** and simulates events such as daily market opening」「or closing.」(原文で行が割れている)。**併せて「Tools to prevent look-ahead bias in the backtesting environment.」も持ち、これは当方に無い。**星は 970、作成は 2019-08-16 | 一次資料 | `https://raw.githubusercontent.com/quarkfin/qf-lib/master/README.md`(2026-09-22 取得)の 16・21 行、`20260922_tools_1_run20.log:159`・`:160`・`:148` |
| 15 | **候補 69 `gobacktest` はイベント駆動で、入力は足である。**README の逐語は「data := &data.BarEventFromCSVFile{FileDir: \"../testdata/test/\"}」と「EventHandler - the different types of events, which travel through this system - data event, signal event, order event and fill event」である。**ただし README の取り込みの逐語は `github.com/dirkolbrich/gobacktest` で、登録の名前 `gobacktest/gobacktest` と食い違う**(移管の可能性。委任文 §6-1 の「配布元の一致」の観点で記録する)。星は 241、最後の押し出しは 2023-10-11 | 一次資料 | `https://raw.githubusercontent.com/gobacktest/gobacktest/main/README.md`(2026-09-22 取得)の 23・37・112 行、`20260922_tools_1_run20.log:596`・`:597`・`:589`・`:194` |
| 16 | **候補 61 `barter-rs` と候補 68 `quanttrader` は、イベント駆動は原典で決まるが、粒度は README では決まらない。**61 の登録情報の逐語は「Open-source Rust framework for building event-driven live-trading & backtesting systems」、68 の README の逐語は「Welcome to quanttrader, a pure python-based event-driven backtest and live trading package for quant traders.」である。**一方、61 の README に `bar` / `tick` / `candle` / `OHLCV` の語は 1 つも無く(`Daily` が 3 回出るだけ)、68 の README にはその 5 語がどれも 1 つも無い。**粒度は README の外(原典の実装か文書)にある。**「無い」と書いているのはこの 2 つの README の中についてだけで、道具に足やティックの口が無いという意味ではない** | 実測 | `20260922_tools_1_run20.log:585`・`:586`・`:587`(README の全文に対する語の数え上げ)・`:135`・`:179` |
| 17 | **候補 41 `prediction-market-backtesting` は板の差分とティックを再生すると書いている。**README の逐語は「- Book replay order book deltas with trade ticks」(「New in Version 3」の節)で、登録情報の説明の逐語は「An extension for Nautilus Trader」である。**既定枝は `v4.1-alpha`、星は 1202、作成は 2026-02-13、最後の押し出しは 2026-05-16。**追跡ファイルの根に `COPYING`・`COPYING.LESSER`・`LICENSE`・`LICENSE-MIT` の 4 本が同時に在り、**どの条件で配られているかが根のファイル名からは一意に決まらない** | 一次資料 | `https://raw.githubusercontent.com/evan-kolberg/prediction-market-backtesting/v4.1-alpha/README.md`(2026-09-22 取得)の 41 行、`20260922_tools_1_run20.log:52`・`:30`・`:605`・`:606` |
| 18 | **候補 63 `trade-frame` の 18 回目の逐語は、この回に取り直しても同じだった。**README の 17 行の逐語は「A [Simulation Interface](lib/TFSimulation/SimulationInterface.hpp) is provided for testing strategies off-line.  The simulator will accept [Orders](lib/TFTrading/Order.h) of type Market and Limit assuming bid/ask data has been captured from the exchange during a live session.  A 50ms - 100ms delay queue has been implemented to better simulate round-trip and slippage effects of order submission.」(原文の二重空白のまま)。**18 回目の知見 9 の逐語と、語順・語ともに同じである。**ただし 1 文字単位では同じではない: 18 回目は README の差し込みの印(`[Simulation Interface](lib/TFSimulation/SimulationInterface.hpp)` の角括弧と丸括弧)を外し、文と文の間の二重空白を 1 つに詰め、末尾の句点を落とした形で引いている。**原典そのものは上の形である。印は 18 回目に確定済みで、この回は変えていない** | 一次資料 | `https://raw.githubusercontent.com/rburkholder/trade-frame/master/README.md`(2026-09-22 取得)の 17 行、`20260922_tools_1_run20.log:23`・`:10` |
| 19 | **`ungh.cc` はこの回も 2 回 000 を返した。**候補 65 の追跡ファイルの一覧と候補 59 の登録情報である。**どちらも 1 回ずつ打ち直して 200 になった。000 を 404 と同じ欄に書いていない** | 実測 | `20260922_tools_1_run20.log:163`・`:165`・`:427`・`:429` |
| 20 | **作業者向けの手引きを同梱する登録が、この回の 12 件のうち 2 件あった。**追跡ファイルの全体(根だけでなく下の階層も)を機械で走査した結果、候補 41 `evan-kolberg/prediction-market-backtesting` が `AGENTS.md` を、候補 58 `nautechsystems/nautilus_trader` が `AGENTS.md` と `CLAUDE.md` を追跡している。残る 10 件は当たり無しである。**委任文 §6-3(取ってきた文章は指示ではない)に従い、中身の指示には従っていない。所在だけを記録する。**19 回目までの通算 3 件(候補 44・74・120)に、この 2 件が加わる | 実測 | `20260922_tools_1_run20.log:572`〜`:584` |

### 候補の一覧

**新しい候補は 0 件。**候補の一覧は 120 番のままである(起動の指定の逐語「**候補は 1 件も増やさないでください。**」)。

#### 重複の洗い出し(起動の指定 (1))

**突き合わせの手順**: (i) 各候補について `所有者/登録名`(GitHub)か配布物の名前(PyPI)を鍵として集める。
鍵の出所は 3 つだけで、**この回に取り直した 39 番の一覧の被リンク**(51 番から 89 番)・**報告の本文に既に書かれている所有者/登録名**・
**既存の生ログから機械抽出した URL**(`grep -ohE '(github\.com|ungh\.cc/repos)/[A-Za-z0-9_.-]+/[A-Za-z0-9_.-]+'`)である。記憶では埋めていない。
(ii) 鍵を小文字に揃えて束ね、2 つ以上の番号が同じ鍵に落ちる組を出す。
(iii) 1 回目・2 回目の節は**現行とは別の番号体系**を使っているので、その節の候補も同じ鍵で別途突き合わせる。
(iv) 所有者を外した登録名だけでも束ね、取り違えやすい組を別に出す。

**結果**:

| 組 | 内容 | 件数 |
|---|---|---|
| 現行 1〜120 の中での重複 | 同じ登録が 2 つ以上の番号を持つ組 | **0** |
| 1・2 回目の独自番号と現行の番号が同じ登録を指す組 | うち 21 件は現行 1〜23 への意図された振り直し | 25 |
| そのうち、到達済みの登録を「未着手」で立て直した組 | 旧 3 = 73 / 旧 2 = 58 / 旧 4 = 75 / 旧 5 = 55 | **4**(新しく出たのは 3) |
| 1・2 回目に在って現行に番号が 1 つも無い登録 | `mhallsmoore/qstrader` / `gbeced/pyalgotrade` / `carlos8f/zenbot` / `ccxt/ccxt` | **4** |
| 突き合わせの鍵が取れなかった候補 | 40・45・46・48・49・50・107・108・109・117・118 | **11** |
| 登録名だけ同じで所有者が違う組 | 39 と 110 / 97 と 102 | 2 |

**鍵が取れなかった 11 件については「重複が無い」と書けない。**この 11 件は GitHub の登録でも PyPI の配布物でもなく、
公式の場の URL が確定していないものを含むため、**機械で突き合わせる鍵そのものが無い**(委任文 §4.1 の `未確認`)。

#### この回に印が決まった候補

**この回は 1 件も深掘りしていないので、下の 10 件はすべて「浅い」である。**

1. `evan-kolberg/prediction-market-backtesting`(41 番) — 状態: **浅い(一次資料に到達し、機能と既定枝と許諾ファイルの在処を取った)。**印は `区分1-イベント駆動` と `区分1-ティック`、および `区分6 へ`。
   **板の差分とティックを再生すると原典が書いている**(知見 17)。**指値の埋まり方の規則は上流の 58 番の側にあるので、`区分1-板の待ち行列` はこの回に付けていない。**
   未確認: 許諾の条件(根に 4 本の許諾ファイルが同時に在る)・料金・導入・最小実行・外部送信・依存。
2. `QuantConnect`(52 番、`QuantConnect/Lean`) — 状態: **浅い(README と約定の模型の原典から粒度を確定)。**印は `区分1-イベント駆動` と `区分1-足` と `区分1-ティック`、および `区分2 へ`。
   **約定の模型が `Tick` と `TradeBar` と `QuoteBar` で分岐する**(知見 10)。**22 番 `Lean CLI` は同じ機関の別の登録**(`QuantConnect/lean-cli`)**で、鍵が違うので重複ではない。**
   未確認: 許諾の中身・料金・導入・最小実行・当方のデータの投入・外部送信。
3. `nautilus_trader`(58 番) — 状態: **浅い(README から 4 要素を確定)。**印は `区分1-イベント駆動` と `区分1-ティック` と `区分1-足`、および `区分2 へ`。
   **検証の入力に気配のティック・約定のティック・足・板を同時に挙げている**(知見 8)。**板の待ち行列の位置を模擬するかは README に書かれていない**(知見 9)ので `区分1-板の待ち行列` は付けていない。
   **1 回目の節で到達済みの登録である**(知見 3・4)。未確認: 許諾・料金・導入・最小実行・当方のデータの投入。
4. `barter-rs`(61 番) — 状態: **浅い(登録情報でイベント駆動を確定。粒度は README では決まらない)。**印は `区分1-イベント駆動`、および `区分2 へ`。
   **README に足・ティック・ローソクの語が 1 つも無い**(知見 16)。未確認: 粒度・許諾・料金・導入・最小実行。
5. `qf-lib`(62 番) — 状態: **浅い(README からイベント駆動と粒度を確定)。**印は `区分1-イベント駆動` と `区分1-足`、および `区分3 へ` と `区分5 へ` と `区分8 へ`。
   **刻むのは日の寄りと引けで、先読みの偏りを防ぐ道具を持つと書いている**(知見 14)。未確認: 許諾・料金・導入・最小実行・データ供給元の鍵の要否。
6. `trade-frame`(63 番) — 状態: **18 回目に確定済み。この回は逐語を取り直して同じであることを確かめただけ**(知見 18)。印は 18 回目のまま `区分1-ティック` と `区分1-イベント駆動` と `区分1-市場影響と約定の模型`。
7. `aat`(65 番、`AsyncAlgoTrading/aat`) — 状態: **浅い(README と板の実装の原典から確定)。**印は `区分1-イベント駆動` と `区分1-板の待ち行列`、および `区分2 へ`。
   **価格帯ごとに注文を到着順の列に積む実装を持つ**(知見 11)。未確認: 粒度(足かティックか)・許諾・料金・導入・最小実行。
8. `quanttrader`(68 番) — 状態: **浅い(README でイベント駆動を確定。粒度は README では決まらない)。**印は `区分1-イベント駆動`、および `区分2 へ`。
   **README に足・ティック・ローソクの語が 1 つも無い**(知見 16)。**最後の押し出しは 2024-06-20 で、39 番の一覧の `dormant since 2024-06` と一致する。**未確認: 粒度・許諾・料金・導入・最小実行。
9. `gobacktest`(69 番) — 状態: **浅い(README からイベント駆動と足を確定)。**印は `区分1-イベント駆動` と `区分1-足`。
   **取り込みの逐語の所有者が登録の所有者と食い違う**(知見 15)。**最後の押し出しは 2023-10-11。**未確認: 許諾・料金・導入・最小実行・食い違いの正体。
10. `PandoraTrader`(59 番) — 状態: **浅い(README の 1 行で粒度を確定)。**印は `区分1-ティック`、および `区分2 へ`。
   **根に許諾ファイルが無い**(知見 13)。未確認: 許諾・料金・導入・最小実行・イベント駆動かどうか。
11. `Oddpool/PredictionMarketBench`(90 番) — 状態: **浅い(README から粒度と待ち行列を確定)。**印は `区分1-ティック` と `区分1-板の待ち行列`、および `区分6 へ`。
   **待ち行列の位置を表示サイズと pro-rata の配分で模擬すると書いている**(知見 12)。**根に許諾ファイルが無い。**未確認: 許諾・料金・導入・最小実行・Kalshi のデータの入手条件。

**6 要素ごとの残り(区分 1 に入ったもののうち、状態がまだ確定していない候補の数)**:
`区分1-足` は 43・44・51・52・53・54・55・57・60・67・70・72・75・87・91・92 と 40・46・48・56・80・85・86 と 73・74・120 に **58・62・69** を足して 29 件。
`区分1-ティック` は 58・90・59・63・119 に **52** を足して 6 件。
`区分1-板の待ち行列` は 96・97・100・109 に **65・90** を足して 6 件。
`区分1-イベント駆動` は 41・52・58・61・62・65・68・69・63 の 9 件(数は変わらないが、**63 を除く 8 件の印の根拠がこの回に一次資料になった**。起動の指定 (2) の本体はこれで済んだ)。
`区分1-ベクトル化` は 73・74 の 2 件(**44 を外した**。19 回目の検収 §4(6))。
`区分1-市場影響と約定の模型` は 106・116・63・119 の 4 件(この回に増減なし)。

**`判別に一次資料が要る` の残り**: 49 番だけ。**登録の内側にあり、当方は登録しない。**
**残りの候補名**: 43 番の浅い部分、51 番から 93 番のうち上で印を付けなかったもの、
94 番・96 番・97 番・100 番から 120 番の浅い部分。
**39 番と 110 番と 112 番の一覧の残りの節からも、まだ名前を抜いていない。**
**`区分1-板の待ち行列` の 4 件(96・97・100・109)には、この回は手を付けていない**(予算。起動の指定の順で 3 番目)。
**`区分1-足` の 26 件にも手を付けていない**(起動の指定の逐語「**`区分1-足` の 26 件には手を付けないでください**」)。

### ツール1件ごとの表

| ツール | 何ができるか | 料金の構造 | 当方に無いもの | この環境での到達 |
|---|---|---|---|---|
| `evan-kolberg/prediction-market-backtesting` | 予測市場の戦略を、板の差分と約定のティックを再生して検証する。58 番の拡張として動く | 未確認(根に許諾ファイルが 4 本あり条件が一意に決まらない。料金の記述は README に無い) | **板の差分そのものを再生の入力にする形** | **可**(原典に到達。導入はしていない) |
| `QuantConnect/Lean` | 足とティックの両方を入力にして、約定の模型を差し替えながらイベント駆動で回す | 未確認(この回は README と約定の模型だけを読んだ) | **約定の模型を差し替え可能な部品として外に出していること**と、足とティックで約定の判定を分ける作り | **可**(原典に到達。導入はしていない) |
| `nautilus_trader` | 気配のティック・約定のティック・足・板を同時に入力にして、同じ機関で検証と実弾を回す | 未確認(この回は README だけを読んだ) | **研究と本番を同じ機関で回すという設計**と、ナノ秒の分解能 | **可**(原典に到達。導入はしていない) |
| `barter-rs` | イベント駆動で実弾・紙・検証を同じ機関で回し、何千の検証を同時に走らせる | 未確認 | **監査の流れ(`AuditStream`)を外の見張りに渡す形** | **可**(原典に到達。導入はしていない) |
| `qf-lib` | 日の寄りと引けをイベントとして刻み、先読みの偏りを防ぐ道具つきで検証する | 未確認(データ供給元に Bloomberg などの有償が含まれるが、条件はこの回に取っていない) | **先読みの偏りを防ぐ道具**と、結果を文書にまとめる雛形 | **可**(原典に到達。導入はしていない) |
| `aat` | 価格帯ごとに注文を到着順の列に積む板を持ち、それで合成の取引所を動かす | 未確認 | **価格帯ごとの到着順の列を持つ板の実装**(当方は参照実装 `maker_fill_ref.py` を持つが研究の模擬に組み込んでいない) | **可**(原典に到達。導入はしていない) |
| `quanttrader` | イベント駆動で検証と実弾を同じ符号で回す | 未確認 | 実弾の口(Interactive Brokers)の注文種別の広さ | **可**(原典に到達。導入はしていない) |
| `gobacktest` | 足の事象を CSV から流し、事象・信号・注文・約定の 4 つの事象で回す | 未確認 | **事象の型を 4 つに分けて明示する作り** | **可**(原典に到達。導入はしていない) |
| `PandoraTrader` | ティック水準の検証を持つ C++ の高頻度の台 | 未確認(根に許諾ファイルが無い) | ティック水準の検証を中国の先物の口と一体で持つこと | **可**(原典に到達。導入はしていない) |
| `Oddpool/PredictionMarketBench` | Kalshi の板の写しと約定の記録を再生し、待ち行列の位置を表示サイズと pro-rata で模擬する | 未確認(根に許諾ファイルが無い。Kalshi のデータの入手条件も未確認) | **待ち行列の位置を模擬すること**と、**pro-rata の配分**(当方の参照実装は FIFO) | **可**(原典に到達。導入はしていない) |

#### §4.0 の機械可読の表

**この回で `[深掘り]` に達した道具は無い。**起動の指定の逐語「**深掘り(構築・導入・最小実行)は要りません。**」に従い、
導入も最小実行も 1 件も行っていない。委任文 §4.0 は「深掘りした道具は、文章より先に表に 1 行ずつ書く」と定めており、
深掘りが 0 件なので **§4.0 の機械可読の表にこの回の行は無い**(18 回目・19 回目と同じ)。
この回に印が決まった候補は、いずれも委任文 §4.0 の語彙の過半が `未確認` であり、規則どおり候補の一覧に「浅い(何が未確認か)」と書いてある。

### 予算

割当は 1 回 5 万トークン・20 分。**起動の指定の (1)・(2) の本体・(3)・(4) は終わった。**
(1) の重複の洗い出しは 120 件全部について機械で打ち、6 つの組に分けて上に出した。
(2) の `区分1-イベント駆動` の 9 件は、63 番(18 回目に確定済み)を除く 8 件の印をこの回に一次資料で確定した。
続けて `区分1-ティック` の 5 件のうち、まだ印が決まっていなかった 59 番と 90 番の 2 件も確定した(58 番と 63 番はこの回と 18 回目で確定、119 番は 18 回目に確定済み)。
**`区分1-板の待ち行列` の 4 件(96・97・100・109)には手が届かなかった。**
(3) の新しい検索計画は打っていない。候補は 1 件も増やしていない。(4) の深掘りは 0 件で、これは起動の指定どおりである。
この回に導入・構築・最小実行・登録・鍵の発行・購入・発注は 1 件も行っていない。

### 原文に無い判断が要った点(リードに渡す)

1. **(a) の直しを、19 回目の節の候補 44 の行に当てた。**候補 44 の印は 14 回目の節と 19 回目の節の 2 箇所に書かれている。
   **現在の状態を述べているのは 19 回目の側**なのでそちらを直し、14 回目の節はその時点の記録として触っていない。
   **ただし 19 回目の節の「6 要素ごとの残り」の行には `区分1-ベクトル化` は 44・73・74 の 3 件 と書かれたままである。**
   起動の指定が許したのは 2 箇所だけなので触っていないが、**そこも直すかはリードが決めること。**
2. **候補 52 に `区分1-足` と `区分1-ティック` を足した。**起動の指定は「印を確定させてください」であって「足してよい」とは書いていない。
   19 回目の検収 §4(2) が 73・74 について「よい」と判断した形に倣った。**同じ理由で 58 に `区分1-足`、62 と 69 に `区分1-足`、65 と 90 に `区分1-板の待ち行列`、41 と 90 に `区分1-ティック` を足した。**
   **足してよいかはリードが決めること。**
3. **候補 58 に `区分1-板の待ち行列` を付けなかった。**README は検証の入力に `order book` を挙げているが、
   **入力として板を受け取ることと、指値の待ち行列の位置を模擬することは別である。**原典の実装まで降りていないので付けなかった。
   **同じ理由で候補 41 にも付けていない**(埋まり方の規則は上流の 58 番の側にある)。**この線引きでよいかはリードが決めること。**
4. **候補 61 と 68 について「README に足・ティックの語が無い」と書いた。**これは **README の中についての測定**であって、
   道具に足やティックの口が無いという主張ではない(CLAUDE.md §0.2 O-2)。**粒度は `未確認` のままである。**
   原典の実装まで降りて粒度を決めるかはリードが決めること。
5. **1・2 回目の節に在って現行に番号が無い 4 件(`qstrader`・`pyalgotrade`・`carlos8f/zenbot`・`ccxt`)を、この回に番号として立てていない。**
   起動の指定の逐語が「**候補は 1 件も増やさないでください。**」なので立てていない。
   **ただし 2 回目の節では `qstrader` と `pyalgotrade` は最小実行まで打っており、候補の一覧から落ちた側である**(委任文 §3-3「候補の一覧から黙って落とさない」)。
   **番号を立て直すかはリードが決めること。**
6. **重複した 3 件(55・58・75)の状態を「未着手」のままにした。**1・2 回目の節では到達済み・実測済みだが、
   **その到達の記録を現行の番号に移すことは、番号の一本化そのものであり、19 回目の検収 §4(1) の「3 番と 73 番は同じ登録。以後 73 番を正とする」と同じ決定が要る。**
   **この回は勝手に移さず、知見 3・4 に逐語で書いて残した。リードが決めること。**
7. **鍵が取れなかった 11 件を「重複なし」に数えていない。**委任文 §4.1 の `未確認` として別立てにした。
   **この 11 件を「一次資料に到達した候補」の数に入れるかどうかは、完了の数の定義そのものなのでリードが決めること。**

### 取ってきた文章の中の作業者向けの指示(委任文 §6-3)

**この回の 12 件のうち 2 件が、作業者向けの手引きを追跡していた**(知見 20)。
候補 41 `evan-kolberg/prediction-market-backtesting` の `AGENTS.md` と、候補 58 `nautechsystems/nautilus_trader` の `AGENTS.md`・`CLAUDE.md` である。
**中身は読んでいないし、従ってもいない。所在だけを記録した。**取ってきた README の中の文にも 1 つも従っていない。
**取り込んだ登録が当方の作業者に指示を出しうる経路は、19 回目までの通算 3 件(候補 44・74・120)にこの 2 件を足して 5 件になった。使う段でオーナーに出す。**
...
## 区分1 — 24 回目の実行(2026-09-22)

委任文: `docs/DATA/delegations/20260922_tools_survey_prompt.md@ce0012c95154`。生ログ: `docs/DATA/probes/20260922_tools_1_run24.log`。
23 回目のリードの検収(`docs/AUDITOR/VERDICTS/2026-09-22_tools_scan_cat1_run23.md`)と、その §9 の起動の指定に従う。

**この回にしたこと**: (1) 22 回目の貼り付けの食い違いの訂正を書いた / (2) `区分1-市場影響と約定の模型` の印を持つ候補すべてに「段」を書き、3 番・8 番・19 番に印を足した /
(3) 23 回目に決まらなかった 7 件(4・5・6・15・18・19・21 番)を原典で埋めた / (4) 印を付けるたびに新しい検査(実行経路が呼んでいるか)を当てた /
(5) 22 番と 52 番、13 番と 123 番に互いへの参照を書いた / (6) 数え直しを 3 段で出した。
**導入・構築・最小実行・登録・鍵の発行・購入・発注はこの回に 1 件も無い。新しい検索計画も打っていない。候補も増やしていない。**

**書き換えの範囲**: `git diff` の削除行は **0 行**である。指定の 1 から 6 はすべて**新しい節への追記だけ**で果たした。
23 回目の検収の逐語「**追記だけで済むならそのほうがよい(記録を壊さない)。**」に従った。

**生ログの本数**: `ls docs/DATA/probes/20260922_tools_1_run*.log | wc -l` の出力は **23**(生ログ `20260922_tools_1_run24.log` の `ls_probes` の節)。
**これはこの回の生ログ `run24.log` を作ったあとに打った値で、内訳は `run2` から `run24` である**(`run1` の生ログは無い。21 回目から 23 回目と同じ)。

**字形について**: この回に逐語で引いた原典に、非分割ハイフンなどの字形違いは**見当たらなかった**(すべて半角の ASCII)。
ただし原文の書き方をそのまま写した箇所が 2 つある: (a) 候補 35 の登録名 `thirupathikannan-ai/Optimal-Execution-And-Market-Impact-Simulator-` は**末尾のハイフンまで登録名の一部**である、
(b) 候補 33 の逐語 `slippage_bps += volume_impact_factor_ * std::sqrt(participation) * 10000.0;` の末尾の分号は原文のとおりである。

**到達の記録**: HTTP 404 は **1 件**で、`ungh.cc/repos/Ziplime/ziplime` である。
**これは所有者名が誤っていたためで、正しい登録は `Limex-com/ziplime`**(6 回目に許諾の取得で当たっていたもの)。打ち直して `code=200` を得た。
**TLS の切断(`code=000`、`rc=35`)は 2 件あり、いずれも打ち直して `code=200` を得ている。404 の欄には入れていない。**
**既定枝は `ungh.cc` の `defaultBranch` で確かめてから取った。当てずっぽうの取得は 0 件である。**
そのとき **`robcarver17/pysystemtrade` が `pst-group/pysystemtrade` に付け替えられていること**と、**既定枝が `develop` であること**が分かった。

### 22 回目の貼り付けの食い違いの訂正(起動の指定 1)

**22 回目の節(報告書 6714 行)に貼られた検査の出力は `K12 検査の出力の貼付 1 件` / `---- 合計 1 件` で、その 4 行下の文
「この回は 13 本すべてが 0 件で通った」と食い違っている。**

**正体は K12 の自己参照である。**K12 は「受け入れ検査の出力の節に貼られた合計が、いま数え直した合計と一致するか」を見るので、
**貼り付ける前に打つと「貼り付けが無い」で 1 件当たり、その出力を貼ると当たりが消える。**
**したがって 22 回目に貼られていたのは、貼り付け前の途中の実行の出力である。**文のほう(13 本すべて 0 件)は最終の状態について正しく、
リードが検収で打ち直して 0 件を確認している(23 回目の検収 §2)。

**22 回目の節は書き換えていない**(過去の回の節は記録である)。**訂正はこの回の節に書く、というリードの指定のとおりにした。**

**以後の扱い(この回から適用)**: 貼るのは**最終の実行の出力**とする。
K12 の自己参照でそれが作れない回は、**「貼ったのは貼り付け前の実行で、K12 の 1 件は自己参照。貼り付け後に打ち直した結果は 0 件」と節に書く。**黙って並べない。
**この回は最終の実行の出力を貼れた**(理由は「受け入れ検査の出力」の節に書く)。

### 検索計画

**この回は検索計画を打っていない。**起動の指定の逐語「**新しい検索計画は打たないでください。**」による。
X の経路も同じ理由で打っていない。**候補も 1 件も増やしていない。**この回に取った GET は、すべて既に一覧に在る候補の実装の原典と、その在処・既定枝の確認である。

### 出典

| # | 出典 | 取得日 | 使った先 |
|---|---|---|---|
| 1 | `https://ungh.cc/repos/robcarver17/pysystemtrade` | 2026-09-22 | 候補 3 の既定枝(`pst-group/pysystemtrade` への付け替えと `develop` が分かった) |
| 2 | `https://ungh.cc/repos/pst-group/pysystemtrade/files/develop` | 2026-09-22 | 候補 3 の在処 |
| 3 | `https://raw.githubusercontent.com/pst-group/pysystemtrade/develop/systems/accounts/order_simulator/fills_and_orders.py` | 2026-09-22 | 候補 3 の約定の作り方 |
| 4 | `https://raw.githubusercontent.com/pst-group/pysystemtrade/develop/systems/accounts/order_simulator/simple_orders.py` | 2026-09-22 | 候補 3 の注文の型 |
| 5 | `https://raw.githubusercontent.com/pst-group/pysystemtrade/develop/systems/accounts/order_simulator/hourly_limit_orders.py` | 2026-09-22 | 候補 3 の指値と次の足の突き合わせ |
| 6 | `https://raw.githubusercontent.com/pst-group/pysystemtrade/develop/sysobjects/fills.py` | 2026-09-22 | 候補 3 の滑りの旗の定義 |
| 7 | `https://raw.githubusercontent.com/pst-group/pysystemtrade/develop/systems/accounts/pandl_calculators/pandl_using_fills.py` | 2026-09-22 | 候補 3 の損益の側(旗を読んでいないこと) |
| 8 | GitHub の code search `price_requires_slippage_adjustment repo:pst-group/pysystemtrade` | 2026-09-22 | 候補 3 の旗を読む側の所在(新しい検査) |
| 9 | `https://ungh.cc/repos/gbeced/basana` と `.../files/develop` | 2026-09-22 | 候補 1 の既定枝(`develop`)と在処 |
| 10 | `https://raw.githubusercontent.com/gbeced/basana/develop/basana/backtesting/liquidity.py` | 2026-09-22 | 候補 1 の影響の関数 |
| 11 | `https://raw.githubusercontent.com/gbeced/basana/develop/basana/backtesting/order_mgr.py` | 2026-09-22 | 候補 1 の実行経路が影響の関数を呼ぶこと |
| 12 | `https://ungh.cc/repos/pmorissette/bt` と `.../files/master` | 2026-09-22 | 候補 5 の既定枝(`master`)と在処 |
| 13 | `https://raw.githubusercontent.com/pmorissette/bt/master/bt/backtest.py` | 2026-09-22 | 候補 5 の走らせ方(ベクトル化の判定) |
| 14 | `https://ungh.cc/repos/Ziplime/ziplime`(HTTP 404)と `https://ungh.cc/repos/Limex-com/ziplime` | 2026-09-22 | 候補 6 の登録の所在と既定枝(`master`) |
| 15 | `https://ungh.cc/repos/Limex-com/ziplime/files/master` | 2026-09-22 | 候補 6 の在処 |
| 16 | `https://raw.githubusercontent.com/Limex-com/ziplime/master/ziplime/gens/domain/simulation_clock.py` | 2026-09-22 | 候補 6 の事象の出し手 |
| 17 | `https://raw.githubusercontent.com/Limex-com/ziplime/master/ziplime/trading/trading_algorithm.py` | 2026-09-22 | 候補 6 の事象の受け手(振り分け) |
| 18 | `https://raw.githubusercontent.com/Limex-com/ziplime/master/ziplime/trading/trading_algorithm_executor.py` | 2026-09-22 | 候補 6 の駆動の入口 |
| 19 | `https://raw.githubusercontent.com/Limex-com/ziplime/master/ziplime/finance/slippage/volume_share_slippage.py` | 2026-09-22 | 候補 6 の影響の関数 |
| 20 | `https://raw.githubusercontent.com/Limex-com/ziplime/master/ziplime/finance/slippage/slippage_model.py` | 2026-09-22 | 候補 6 の滑りの模型の基底 |
| 21 | `https://raw.githubusercontent.com/Limex-com/ziplime/master/ziplime/exchanges/simulation_exchange.py` | 2026-09-22 | 候補 6 の実行経路が滑りの模型を呼ぶこと |
| 22 | `https://ungh.cc/repos/Mendl-Labs/BacktestingCore/files/master` | 2026-09-22 | 候補 15 の在処 |
| 23 | `https://raw.githubusercontent.com/Mendl-Labs/BacktestingCore/master/backtest/src/engine.rs` | 2026-09-22 | 候補 15 の駆動の形 |
| 24 | `https://raw.githubusercontent.com/Mendl-Labs/BacktestingCore/master/backtest/src/simulation.rs` | 2026-09-22 | 候補 15 の模擬の輪(事象の列の有無) |
| 25 | `https://raw.githubusercontent.com/stefan-jansen/zipline-reloaded/main/src/zipline/gens/tradesimulation.py` | 2026-09-22 | 候補 18 の駆動の形 |
| 26 | `https://ungh.cc/repos/jesse-ai/jesse` と `.../files/master` | 2026-09-22 | 候補 19 の既定枝(`master`)と在処 |
| 27 | `https://raw.githubusercontent.com/jesse-ai/jesse/master/jesse/modes/backtest_mode.py` | 2026-09-22 | 候補 19 の約定の起こし方 |
| 28 | `https://raw.githubusercontent.com/jesse-ai/jesse/master/jesse/services/order_service.py` | 2026-09-22 | 候補 19 の埋まる量 |
| 29 | `https://raw.githubusercontent.com/jesse-ai/jesse/master/jesse/models/FuturesExchange.py` | 2026-09-22 | 候補 19 の建玉と手数料の側 |
| 30 | `https://ungh.cc/repos/microsoft/qlib/files/main` | 2026-09-22 | 候補 21 の在処 |
| 31 | `https://raw.githubusercontent.com/microsoft/qlib/main/qlib/backtest/executor.py` | 2026-09-22 | 候補 21 の進め方(ベクトル化の判定) |
| 32 | `https://raw.githubusercontent.com/microsoft/qlib/main/qlib/backtest/exchange.py` | 2026-09-22 | 候補 21 の影響の式と、実行経路がそれを使うこと |
| 33 | `https://raw.githubusercontent.com/edtechre/pybroker/master/src/pybroker/vect.py` | 2026-09-22 | 候補 4 の `vect.py` の中身 |
| 34 | `https://raw.githubusercontent.com/edtechre/pybroker/master/src/pybroker/strategy.py` | 2026-09-22 | 候補 4 の検証の輪 |
| 35 | `https://raw.githubusercontent.com/edtechre/pybroker/master/src/pybroker/slippage.py` | 2026-09-22 | 候補 4 の影響の式 |
| 36 | `https://ungh.cc/repos/FlashAlpha-lab/flashalpha-fill-simulator` と `.../files/master` | 2026-09-22 | 候補 32 の既定枝(`master`)と在処 |
| 37 | `https://raw.githubusercontent.com/FlashAlpha-lab/flashalpha-fill-simulator/master/src/fillsim/entry.py` | 2026-09-22 | 候補 32 の約定の条件 |
| 38 | `https://ungh.cc/repos/SarthakDalmia1/backtesting_execution_simulator` と `.../files/main` | 2026-09-22 | 候補 33 の既定枝(`main`)と在処 |
| 39 | `https://raw.githubusercontent.com/SarthakDalmia1/backtesting_execution_simulator/main/cpp/execution/slippage_model.cpp` | 2026-09-22 | 候補 33 の影響の式 |
| 40 | `https://raw.githubusercontent.com/SarthakDalmia1/backtesting_execution_simulator/main/cpp/execution/execution_simulator.cpp` | 2026-09-22 | 候補 33 の実行経路が影響の式を呼ぶこと |
| 41 | `https://ungh.cc/repos/thirupathikannan-ai/Optimal-Execution-And-Market-Impact-Simulator-` と `.../files/main` | 2026-09-22 | 候補 35 の既定枝(`main`)と在処 |
| 42 | `https://raw.githubusercontent.com/thirupathikannan-ai/Optimal-Execution-And-Market-Impact-Simulator-/main/main.py` | 2026-09-22 | 候補 35 の影響の引数 |
| 43 | `https://ungh.cc/repos/shubhamcodez/Market-Impact-Model` と `.../files/main` | 2026-09-22 | 候補 36 の既定枝(`main`)と在処 |
| 44 | `https://raw.githubusercontent.com/shubhamcodez/Market-Impact-Model/main/impactModel.py` | 2026-09-22 | 候補 36 の影響の式と当てはめ |
| 45 | `https://ungh.cc/repos/microsoft/MarS` と `.../files/main` | 2026-09-22 | 候補 38 の既定枝(`main`)と在処 |
| 46 | `https://raw.githubusercontent.com/microsoft/MarS/main/market_simulation/examples/market_impact.py` | 2026-09-22 | 候補 38 の影響の出し方 |
| 47 | `https://ungh.cc/repos/rburkholder/trade-frame` と `.../files/master` | 2026-09-22 | 候補 63 の既定枝(`master`)と在処 |
| 48 | `https://raw.githubusercontent.com/rburkholder/trade-frame/master/lib/TFSimulation/SimulateOrderExecution.cpp` | 2026-09-22 | 候補 63 の約定の作り方 |
| 49 | `https://raw.githubusercontent.com/vnpy/vnpy_ctastrategy/main/vnpy_ctastrategy/backtesting.py` | 2026-09-22 | 候補 20 の未約定の保持 |
| 50 | `https://raw.githubusercontent.com/Superalgos/Superalgos/master/Projects/Algorithmic-Trading/TS/Bot-Modules/Trading-Bot/Low-Frequency-Trading/OrdersSimulations.js` | 2026-09-22 | 候補 7 の部分約定の割合の出し方 |

### 知見

| # | 知見 | 印 | 根拠 |
|---|---|---|---|
| 1 | **候補 3 `PySystemtrade` の約定は、指値と「次の足の値」を突き合わせ、埋まらなかったものを空の約定として勘定に残す。**逐語は `market_price=data_for_idx.next_price,` と `limit_price=data_for_idx.current_price,` と `if limit_price > market_price:` と `return empty_fill(fill_datetime)`。**同じファイルに `volume` `order_size` `queue` の当たりは 0 件である**(量も列も見ない)。**これが段 2 の実装である** | 実測 | `.../fills_and_orders.py` と `.../hourly_limit_orders.py` 取得日 2026-09-22 / `20260922_tools_1_run24.log` の `evidence_run24` の `c3` の節 |
| 2 | **候補 3 は、約定ごとに「この値段に滑りを当てるか」の旗を持ち、その旗を読む側が実在する。**逐語は `price_requires_slippage_adjustment: bool = False`(指値の約定)と `price_requires_slippage_adjustment=True,`(成行の約定)で、読む側は `pandl_cash_costs.py` の `include_slippage = fill.price_requires_slippage_adjustment` である。**指値で埋まった約定には滑りを当てず、成行には当てるという作りで、量は見ない** | 実測 | GitHub の code search(`repo:pst-group/pysystemtrade`、`total_count=4`)取得日 2026-09-22 / `20260922_tools_1_run24.log` の `github_code_search` の節 |
| 3 | **候補 1 `Basana` の流動性の模型は、参加率の 2 乗で値段を動かす影響の関数である。**逐語は `The price impact is calculated by multiplying the price impact constant by the square of the ratio of the used volume to the total volume.` と `ret = used_pct ** Decimal(2) * self._price_impact_pct` と `volume_limit_pct: Decimal = Decimal("25")` と `price_impact: Decimal = Decimal("10")`。**実行経路が呼んでいる**: `order_mgr.py` の `liquidity_strategy.on_bar(bar_event.bar)`・`fill = order.try_fill(bar, liquidity_strategy)`・`liquidity_strategy.take_liquidity(abs(balance_updates[bar.pair.base_symbol]))` | 実測 | `.../liquidity.py` と `.../order_mgr.py` 取得日 2026-09-22 / `20260922_tools_1_run24.log` の `evidence_run24` の `c1` の節 |
| 4 | **候補 6 `Ziplime` は、型を持つ事象の列で駆動され、その事象を振り分けている。**出し手の逐語は `yield session, SimulationEvent.SESSION_START` と `yield minute, SimulationEvent.BAR`、受け手の逐語は `if action == SimulationEvent.BAR:` と `elif action == SimulationEvent.SESSION_START:` と `elif action == SimulationEvent.SESSION_END:` である。**登録の所在は `Limex-com/ziplime`** で、`Ziplime/ziplime` は HTTP 404 である | 実測 | `.../simulation_clock.py` と `.../trading_algorithm.py` 取得日 2026-09-22 / `20260922_tools_1_run24.log` の `evidence_run24` の `c6` の節 |
| 5 | **候補 6 の滑りの模型は参加率の 2 乗の影響の関数で、検証の取引所がそれを呼ぶ。**模型の逐語は `price * (1 + price_impact * (volume_share ** 2))` と `volume_limit=DEFAULT_EQUITY_VOLUME_SLIPPAGE_BAR_LIMIT,` と `price_impact=0.1,`、呼び出しの逐語は `slippage = self.get_slippage_model(asset=asset)` と `async for order, txn in slippage.simulate(exchange=self,` である | 実測 | `.../volume_share_slippage.py` と `.../simulation_exchange.py` 取得日 2026-09-22 / `20260922_tools_1_run24.log` の `evidence_run24` の `c6` の節 |
| 6 | **候補 15 `Mendl-Labs/BacktestingCore` は事象で駆動されていない。**`backtest/src/engine.rs` の `VecDeque` `EventQueue` `dispatch(` の当たりは **0 件**で、検証の入口の逐語は `let mut sim_loop = SimulationLoop::new();` と `market_data: &[MarketData],` である。**駆動は市場データの並びを頭から進めるだけで、事象の型も待ち行列も無い** | 実測 | `.../backtest/src/engine.rs` と `.../backtest/src/simulation.rs` 取得日 2026-09-22 / `20260922_tools_1_run24.log` の `evidence_run24` の `c15` の節 |
| 7 | **候補 18 `zipline-reloaded` は、型を持つ事象の列で駆動され、その事象を振り分けている。**逐語は `for dt, action in self.clock:` と `if action == BAR:` と `elif action == SESSION_START:` で、型は `BAR` `SESSION_START` `SESSION_END` `MINUTE_END` `BEFORE_TRADING_START_BAR` の 5 つである | 実測 | `.../src/zipline/gens/tradesimulation.py` 取得日 2026-09-22 / `20260922_tools_1_run24.log` の `evidence_run24` の `c18` の節 |
| 8 | **候補 19 `Jesse` の約定は、足の高値と安値が指値を含むかだけで決まり、埋まる量は注文の量そのものである。**逐語は `if low <= order_price <= high:` と `if candle_service.candle_includes_price(current_temp_candle, order.price):` と `order.filled_qty = order.qty`。**`jesse/modes/backtest_mode.py` の `slippage` の当たりは 0 件**(追跡ファイルの一覧でも 0 件 = 23 回目の知見 13)。**未約定の注文は `active_orders` に残り、次の足でまた突き合わせられる** | 実測 | `.../backtest_mode.py` と `.../order_service.py` 取得日 2026-09-22 / `20260922_tools_1_run24.log` の `evidence_run24` の `c19` の節 |
| 9 | **候補 21 `Qlib` はベクトル化されていない。**逐語は `for order in self._get_order_iterator(trade_decision):` と `trade_val, trade_cost, trade_price = self.trade_exchange.deal_order(` で、**注文を 1 件ずつ Python の繰り返しで処理する。**いっぽう影響の式は参加率の 2 乗で、逐語は `adj_cost_ratio = self.impact_cost * (trade_val / total_trade_val) ** 2` である | 実測 | `.../qlib/backtest/executor.py` と `.../qlib/backtest/exchange.py` 取得日 2026-09-22 / `20260922_tools_1_run24.log` の `evidence_run24` の `c21` の節 |
| 10 | **候補 4 `PyBroker` はベクトル化されていない。**検証の輪の逐語は `for i, date in enumerate(test_dates):` と `portfolio.incr_bars(date, price_scope)` で、**足を 1 本ずつ進める。**`vect.py` の先頭の逐語は `"""Contains vectorized utility functions."""` だが、中身は `@njit(cache=True)` を付けた `_rolling_sum_njit` `_rolling_min_njit` `atr` などの**指標の部品**で、損益と建玉の計算ではない | 実測 | `.../src/pybroker/strategy.py` と `.../src/pybroker/vect.py` 取得日 2026-09-22 / `20260922_tools_1_run24.log` の `evidence_run24` の `c4` の節 |
| 11 | **候補 4 の出来高の滑りの模型は参加率の 2 乗の影響の関数である。**逐語は `"""Volume-based participation cap and square-law price impact.` と 逐語 impact is ``price_impact * (filled_shares / bar_volume) ** 2``(**原文の二重のバッククォートは RST の記法で、そのまま写した**)と `price_impact: float = 0.1,` と `volume_limit: Optional[float] = 0.025,`。**ただし検証が受け取る `slippage_model` の既定は `None` なので、指定しなければ滑りも影響も掛からない**(23 回目の知見 12) | 一次資料 | `.../src/pybroker/slippage.py` 取得日 2026-09-22 / `20260922_tools_1_run24.log` の `c4_slip` の行 |
| 12 | **候補 5 `bt` はベクトル化されていない。**逐語は `for dt in self.dates[1:]:` と `self.strategy.update(dt)` と `self.strategy.run()` で、**日付を 1 つずつ進める繰り返しである** | 実測 | `.../bt/backtest.py` 取得日 2026-09-22 / `20260922_tools_1_run24.log` の `evidence_run24` の `c5` の節 |
| 13 | **候補 20 `VnPy` は未約定の指値を足をまたいで残す。**逐語は `self.active_limit_orders: dict[str, OrderData] = {}` と `for order in list(self.active_limit_orders.values()):` と `if order.vt_orderid in self.active_limit_orders:`。**滑りは定額で、量とは無関係に積まれる**(23 回目の知見 6 の逐語 `self.slippage += trade.volume * size * slippage`。`slippage` は設定から来る 1 単位あたりの値) | 実測 | `.../vnpy_ctastrategy/backtesting.py` 取得日 2026-09-22 / `20260922_tools_1_run24.log` の `c20_bt` の行 |
| 14 | **候補 7 `Superalgos` の部分約定は、量ではなく設定した確率で決まる。**逐語は `let percentageFilled = tradingSystemOrder.simulatedExchangeEvents.simulatedPartialFill.config.fillProbability * 100` と `tradingEngineOrder.orderStatistics.percentageFilled.value = 100`、約定の条件の逐語は `if (tradingEngine.tradingCurrent.tradingEpisode.candle.min.value <= tradingEngineOrder.orderStatistics.actualRate.value) {`。**足の出来高を読む箇所は無い** | 実測 | `.../OrdersSimulations.js` 取得日 2026-09-22 / `20260922_tools_1_run24.log` の `c7_sim` の行 |
| 15 | **候補 32 `flashalpha-fill-simulator` の約定は、気配が指値を余裕つきで越えたかだけで決まる。**逐語は `if combo_bid >= cand.limit_credit + cfg.fill_epsilon:` と `- ``simulate_fill(bar_ts, chain, candidates, config)`` — **the per-bar primitive**.`。**量も列も見ない** | 実測 | `.../src/fillsim/entry.py` 取得日 2026-09-22 / `20260922_tools_1_run24.log` の `evidence_run24` の `c32` の節 |
| 16 | **候補 33 `backtesting_execution_simulator` は平方根の影響の関数を持ち、実行経路がそれを呼ぶ。**模型の逐語は `slippage_bps += volume_impact_factor_ * std::sqrt(participation) * 10000.0;` と `Price SquareRootImpactModel::calculate_slippage(`、呼び出しの逐語は `execution_price = slippage_model_->get_execution_price(`。**ただし既定は `, slippage_model_(std::make_unique<ZeroSlippageModel>())` で、差し替えなければ滑りは 0 である** | 実測 | `.../slippage_model.cpp` と `.../execution_simulator.cpp` 取得日 2026-09-22 / `20260922_tools_1_run24.log` の `evidence_run24` の `c33` の節 |
| 17 | **候補 35 は一時的影響・恒久的影響・べき指数を引数として持つ。**逐語は `temporary_impact=0.01,` と `permanent_impact=0.0001,` と `impact_exponent=1.0,` である | 実測 | `.../main.py` 取得日 2026-09-22 / `20260922_tools_1_run24.log` の `evidence_run24` の `c35` の節 |
| 18 | **候補 36 `Market-Impact-Model` は、影響の関数を実データに当てはめる側である。**逐語は `def temp_impact(x,eta,beta):` と `temporary_impact = eta * sigma * np.sign(x) * np.power(np.abs(x) , beta)` と `popt, pcov = curve_fit(temp_impact, x, y)`。**検証の機関ではなく、係数 eta と beta を推定する当てはめの手続きである** | 実測 | `.../impactModel.py` 取得日 2026-09-22 / `20260922_tools_1_run24.log` の `evidence_run24` の `c36` の節 |
| 19 | **候補 38 `MarS` の市場影響は、関数ではなく模擬から出てくる。**逐語は `target_volume=task.twap_agent_target_volume,` と `max_passive_volume_ratio=0.9,` と `2. Market impact simulations with TWAP agent targeting a volume` で、**取引する代理人を入れた走りと入れない走りを比べて影響を出す。**影響の係数を持つ関数は読んだ範囲に無い | 実測 | `.../market_simulation/examples/market_impact.py` 取得日 2026-09-22 / `20260922_tools_1_run24.log` の `evidence_run24` の `c38` の節 |
| 20 | **候補 63 `trade-frame` の約定は、気配の提示量で頭打ちになる部分約定である。**逐語は `quanApplied = std::min<Trade::tradesize_t>( nOrderQuanRemaining, quote.AskSize() );` と `quanApplied = std::min<Trade::tradesize_t>( nOrderQuanRemaining, quote.BidSize() );` と `: m_dtQueueDelay( milliseconds( 250 ) )`。**注文の遅れの待ち行列は持つが、板の中で自分の前に何枚並んでいるかは追わない** | 実測 | `.../lib/TFSimulation/SimulateOrderExecution.cpp` 取得日 2026-09-22 / `20260922_tools_1_run24.log` の `evidence_run24` の `c63` の節 |

### 候補の一覧

**新しい候補は 0 件。**この回に状態が変わったのは 3・4・5・6・8・13・15・18・19・21・22・52・123 番と、段を書いた 26 件である。
番号と名前は既存の一覧のものをそのまま使う。**この回は 1 件も深掘りしていない。**

#### `区分1-市場影響と約定の模型` の印を持つ候補と、その「段」(起動の指定 2)

**この印の下限はリードが決めた**(23 回目の検収 §4 の逐語「**決定: 印は付ける。そのうえで「どの段か」を必ず書く。**」
「**『跨いだら埋まる』を模型と呼ばないなら、当方の `engine.py` にも約定の模型が無いことになる。**」)。
**段の定義は同じ検収 §4 の表のとおりで、この回に足した候補もその定義に当てた。**
**「段」は、その道具が持ち、かつ検証の実行経路が呼ぶもののうち最も高いものを書く。既定値で無効になるものは注記に書く。**

| 段 | 候補 | 段を決めた実装の逐語(出所) |
|---|---|---|
| 1 | 8 `OpenTrader`(**この回に印を付けた**) | `if (candlestick.close <= smartTrade.entryOrder.price!) {` と `filledPrice: candlestick.close,`。同じファイルの `slippage` `queue` `volume` の当たりは 0 件(23 回目の知見 10) |
| 1 | 32 `FlashAlpha-lab/flashalpha-fill-simulator` | `if combo_bid >= cand.limit_credit + cfg.fill_epsilon:`(この回の知見 15)。**余裕 `fill_epsilon` と気配幅の絞りを足した越え判定で、量も列も見ない** |
| 1 | 7 `Superalgos` | `if (tradingEngine.tradingCurrent.tradingEpisode.candle.min.value <= tradingEngineOrder.orderStatistics.actualRate.value) {`(この回の知見 14)。**定率の滑りと、設定した確率での部分約定を足すが、どちらも量を読まない。段 4 の「量を見る」には当たらない**(リードへの問い 1) |
| 2 | 3 `PySystemtrade`(**この回に印を付けた**) | `market_price=data_for_idx.next_price,` と `if limit_price > market_price:` と `return empty_fill(fill_datetime)`(この回の知見 1)。**成行にだけ滑りを当てる旗を持ち、読む側も在る**(知見 2) |
| 2 | 19 `Jesse`(**この回に印を付けた**) | `if low <= order_price <= high:` と `order.filled_qty = order.qty` と `active_orders`(この回の知見 8)。**足の中で複数の約定が起きるときは足を分割して順番を作るが、量も滑りも見ない** |
| 2 | 20 `VnPy` | `self.active_limit_orders: dict[str, OrderData] = {}` と `for order in list(self.active_limit_orders.values()):`(この回の知見 13)、`long_cross_price = self.bar.low_price` と `self.slippage += trade.volume * size * slippage`(23 回目の知見 6)。**滑りは定額で、量とは無関係** |
| 4 | 2 `Backtrader` | `'''Sets a volume filler for volume filling execution'''` と `# Execution depends on volume filler`(23 回目の知見 4)。**既定は `('slip_perc', 0.0),` `('slip_fixed', 0.0),` と充当器なしで、充当器を与えたときに段 4 になる** |
| 4 | 13 `DeviaVir/zenbot` | `let size = Math.min(buy_order.remaining_size, trade.size)` と `c.avg_slippage_pct = process.env.ZENBOT_AVG_SLIPPAGE_PCT \|\| 0.045`(23 回目の知見 5) |
| 4 | 123 `carlos8f/zenbot` | `Math.min(buy_order.remaining_size, trade.size)` と既定 `0.045`(22 回目の知見)。**13 番と同じ実装の系統である**(下の「13 番と 123 番」の項) |
| 4 | 63 `trade-frame` | `quanApplied = std::min<Trade::tradesize_t>( nOrderQuanRemaining, quote.AskSize() );`(この回の知見 20)。**気配の提示量で頭打ちになる部分約定。列の位置は追わない** |
| 5 | 23 `hftbacktest` | `Order fill simulation that takes into account the order queue position, using provided models or your own custom model` と、最小実行の逐語 `BacktestAsset().data([...]).risk_adverse_queue_model()...`(1 回目の節、報告書 212 行) |
| 5 | 98 `mihircoding/limitOrderBook` | `Price-time priority matching engine and a zero-intelligence order flow simulator.` と、16 回目の最小実行「**先行者の買い 50 枚の後ろに当方の買い 10 枚を並べ、売り 50 枚→売り 10 枚と当てた。先行者が先に食われ、当方が残ることを確かめた**」(報告書 5108 行) |
| 5 | 38 `microsoft/MarS` | `target_volume=task.twap_agent_target_volume,` と `max_passive_volume_ratio=0.9,`(この回の知見 19)。**注文の単位で板に当てる模擬で、影響は関数ではなく比較から出る。段 6 には当たらない** |
| 6 | 1 `Basana` | `ret = used_pct ** Decimal(2) * self._price_impact_pct`(この回の知見 3) |
| 6 | 4 `PyBroker` | 逐語 impact is ``price_impact * (filled_shares / bar_volume) ** 2``(この回の知見 11)。**既定の `slippage_model` は `None`** |
| 6 | 6 `Ziplime` | `price * (1 + price_impact * (volume_share ** 2))`(この回の知見 5) |
| 6 | 15 `Mendl-Labs/BacktestingCore` | `/// Estimate market impact using a square-root model:` と、実行経路が呼ぶ `self.calculate_market_impact(order_size, order_price, avg_volume, config, timestamp)`(23 回目の知見 8) |
| 6 | 18 `zipline-reloaded` | `price * (1 + price_impact * (volume_share ** 2))` と `DEFAULT_EQUITY_VOLUME_SLIPPAGE_BAR_LIMIT = 0.025`(23 回目の知見 3) |
| 6 | 21 `Qlib` | `adj_cost_ratio = self.impact_cost * (trade_val / total_trade_val) ** 2`(この回の知見 9) |
| 6 | 33 `SarthakDalmia1/backtesting_execution_simulator` | `slippage_bps += volume_impact_factor_ * std::sqrt(participation) * 10000.0;` と `Price SquareRootImpactModel::calculate_slippage(`(この回の知見 16)。**照合の機関(段 5 相当)も持つが、既定の滑りは `ZeroSlippageModel`** |
| 6 | 35 `thirupathikannan-ai/Optimal-Execution-And-Market-Impact-Simulator-` | `temporary_impact=0.01,` と `permanent_impact=0.0001,` と `impact_exponent=1.0,`(この回の知見 17) |
| 6 | 36 `shubhamcodez/Market-Impact-Model` | `temporary_impact = eta * sigma * np.sign(x) * np.power(np.abs(x) , beta)` と `popt, pcov = curve_fit(temp_impact, x, y)`(この回の知見 18)。**測る側(当てはめ)である** |
| 6 | 105 `DaniyalMlk/slippage` | 16 回目の最小実行「**合成の値で線形の市場影響・べき乗則の市場影響・Almgren-Chriss の最適軌道・等速の軌道・平方根則の頂点の影響を計算した**」(報告書 5194 行) |
| 6 | 106 `almgren-chriss` | `Permanent impact slope`(gamma)・`Temporary impact slope`(eta)・`Temporary impact intercept`(epsilon)(19 回目の知見 21) |
| 6 | 116 `braverock/blotter` の `acOptTxns` | `\eqn{g(v_{k})} is the linear permanent impact function` と `with \eqn{h(v_{k})}, the linear temporary impact function, defined as:`(19 回目の知見 22) |
| 6 | 119 `peernagy/lob_bench`(**測る側**) | 18 回目の節(報告書 5751 行)の逐語「**検証の機関ではなく、生成した板の列を測る物差しである。**」 |

**当方の位置(比較の対象として)**: `src/bot/backtest/engine.py` は**段 1**(通り抜けた約定でいつでも埋まる)、
`scripts/qa/maker_fill_ref.py` は**段 5**(表示サイズの後ろに並び FIFO で埋まる)。
**23 回目の検収 §4 の逐語「この比較こそが区分 1 の目的である」のとおり、当方も同じ物差しに載せた。**

**この表の候補の数は 26 で、内訳は 23 回目までの 23 件に、この回に印を付けた 3 件(3・8・19 番)を足したものである。**

#### 新しい検査 — その機構を、検証の実行経路が実際に呼んでいるか(起動の指定 4)

**23 回目の検収 §5 の逐語**「**その機構を、検証の実行経路が実際に呼んでいるか。呼び出しの当たりが 0 件なら、印は付けず、所見として書く。**」

| 候補 | 検査した機構 | 結果 | 逐語(呼び出しの側) |
|---|---|---|---|
| 3 `PySystemtrade` | 滑りの旗 `price_requires_slippage_adjustment` | **呼んでいた** | `include_slippage = fill.price_requires_slippage_adjustment`(`pandl_cash_costs.py`。GitHub の code search、`total_count=4`) |
| 8 `OpenTrader` | 足を受け取る模擬器 | **呼んでいた** | `nextCandle(candlestick: ICandlestick) {` が検証の入口(23 回目の知見 10) |
| 19 `Jesse` | 約定の判定と埋まる量 | **呼んでいた** | `_simulate_price_change_effect(` → `if candle_service.candle_includes_price(current_temp_candle, order.price):` → `order_service.execute_order(order)` |
| 1 `Basana` | 流動性・影響の模型 | **呼んでいた** | `fill = order.try_fill(bar, liquidity_strategy)` と `liquidity_strategy.take_liquidity(...)` |
| 6 `Ziplime` | 滑りの模型 | **呼んでいた** | `slippage = self.get_slippage_model(asset=asset)` と `async for order, txn in slippage.simulate(exchange=self,` |
| 21 `Qlib` | 影響の式 | **呼んでいた** | `self.trade_exchange.deal_order(` → `_calc_trade_info_by_order` → `adj_cost_ratio = self.impact_cost * (trade_val / total_trade_val) ** 2` |
| 4 `PyBroker` | 滑りの模型 | **呼んでいた。ただし既定では無効** | `slippage_model` が `strategy.py` に 19 行(23 回目の知見 12)。既定は `slippage_model: Optional[SlippageModel] = None,` |
| 33 `backtesting_execution_simulator` | 影響の式 | **呼んでいた。ただし既定では無効** | `execution_price = slippage_model_->get_execution_price(`。既定は `, slippage_model_(std::make_unique<ZeroSlippageModel>())` |
| 15 `Mendl-Labs/BacktestingCore` | 合成の板 `synthetic_book.rs` | **呼んでいなかった**(23 回目に確定) | `simulation.rs` から `synthetic_book` と `SyntheticOrderBook` の当たりは 0 件。実行経路が呼ぶのは自分の側の `self.calculate_market_impact(...)` |
| 107 `sigc` | 費用・影響の模型 `CostModel` | **呼んでいなかった**(22 回目に確定) | `backtest.rs` から `CostModel` を呼ぶ当たりは 0 件。差し引くのは `period_return -= turnover * self.cost_bps / 10000.0;` だけ |
| 20 `VnPy` | 事象の機関 `EventEngine` | **呼んでいなかった**(23 回目に確定) | `vnpy_ctastrategy/backtesting.py` の `EventEngine` `Event(` `event_engine` の当たりは 0 件 |
| 2 `Backtrader` | 出来高の充当器 | **呼んでいた。ただし既定では無し** | `# Execution depends on volume filler`。既定は充当器なし・`('slip_perc', 0.0),` |
| 7 `Superalgos` | 部分約定と滑り | **呼んでいた** | `OrdersSimulations.js` が模擬そのもので、`percentageFilledSimulation` と `actualRateSimulation` を外に出している |
| 13 `DeviaVir/zenbot` / 123 `carlos8f/zenbot` | 模擬の取引所の約定 | **呼んでいた** | `extensions/exchanges/sim/exchange.js` が `--paper` の経路の取引所(23 回目の知見 5) |
| 18 `zipline-reloaded` | 滑りの模型 | **呼んでいた** | `for order, txn in slippage.simulate(bar_data, asset, asset_orders):`(23 回目の知見 3) |
| 63 `trade-frame` | 約定の模擬 | **呼んでいた** | `SimulateOrderExecution.cpp` の `ProcessOrderQueues( quote );` が気配ごとに呼ばれる入口 |
| 32 `flashalpha-fill-simulator` | 約定の判定 | **呼んでいた** | `simulate_fill(bar_ts, chain, candidates, config)` が道具の公開の入口(`the per-bar primitive`) |
| 23 `hftbacktest` / 98 `limitOrderBook` / 105 `DaniyalMlk/slippage` | 列の模型 / 照合の機関 / 影響の関数 | **呼んでいた**(最小実行で確かめた) | 23 番は 1 回目、98 番と 105 番は 16 回目の最小実行の出力 |
| 35 / 36 / 106 / 116 / 119 | 影響の関数 | **該当なし** | **道具の中に検証の機関が無い**(単体の模型・当てはめ・物差しである)。呼び出しの検査を当てる先が無い |
| 38 `microsoft/MarS` | 影響の出し方 | **未確認** | 13 回目に「この環境では動かせない」と確定しており、実行経路の読みはこの回にしていない |

**この検査でこの回に新しく見つかった「呼んでいない」は 0 件である。**既知の 3 件(15・107・20 番)を数え直しただけで、**新しい型の追加は無い。**

#### 23 回目に決まらなかった 7 件(起動の指定 3)

**7 件すべてが決まった。**

| 候補 | 決まらなかった要素 | この回の決着 | 根拠 |
|---|---|---|---|
| 4 `PyBroker` | `区分1-ベクトル化` | **付けない** | この回の知見 10。検証の輪は `for i, date in enumerate(test_dates):` で、`vect.py` は指標の部品 |
| 5 `bt` | `区分1-ベクトル化` | **付けない** | この回の知見 12。`for dt in self.dates[1:]:` |
| 6 `Ziplime` | `区分1-イベント駆動` | **付ける** | この回の知見 4。`yield minute, SimulationEvent.BAR` と `if action == SimulationEvent.BAR:` |
| 15 `Mendl-Labs/BacktestingCore` | `区分1-イベント駆動` | **付けない** | この回の知見 6。`engine.rs` の `VecDeque` `EventQueue` `dispatch(` の当たりは 0 件 |
| 18 `zipline-reloaded` | `区分1-イベント駆動` | **付ける** | この回の知見 7。`for dt, action in self.clock:` と 5 つの事象の型への振り分け |
| 19 `Jesse` | `区分1-市場影響と約定の模型` | **付ける(段 2)** | この回の知見 8 |
| 21 `Qlib` | `区分1-ベクトル化` | **付けない** | この回の知見 9。`for order in self._get_order_iterator(trade_decision):` |

**11 番 `OctoBot` の 5 要素は `未確認` のまま残した**(`区分1-ティック`・`区分1-板の待ち行列`・`区分1-イベント駆動`・`区分1-ベクトル化`・`区分1-市場影響と約定の模型`)。
起動の指定の逐語「**模擬を回すのは深掘りで、この段では範囲の外です。**」「**完了の報告でこの 5 要素を「尽きた」には数えません。**」に従った。

#### 22 番 `Lean CLI` と 52 番 `QuantConnect`(起動の指定 5)

**互いへの参照**:
- **22 番 `Lean CLI` は、52 番 `QuantConnect`(`QuantConnect/Lean`)の検証の機関を起動するための入口である。**22 番そのものに検証の機関は無く、機関は取得していない Docker 画像の中にある(3 回目の節、報告書 1397〜1401 行)。
- **52 番 `QuantConnect` は、その機関の側である。**印は `区分1-足` と `区分1-イベント駆動`(22 回目の節、報告書 6109 行)。
- **番号は両方残す**(登録・入口としては別物であり、一覧から消さない)。**ただし数えるときは機関として 1 つである。**

#### 13 番 `DeviaVir/zenbot` と 123 番 `carlos8f/zenbot`(起動の指定 5)

**互いへの参照**:
- **123 番 `carlos8f/zenbot` が本家で、13 番 `DeviaVir/zenbot` はその分岐である**(20 回目の洗い出し)。
- **印の根拠は同じ実装の系統から来ている。**13 番で当てた逐語(`eventBus.on('trade', queueTrade)`・`Math.min(buy_order.remaining_size, trade.size)`)は、
  123 番の同じ位置の実装と同じものである。13 番だけの違いは滑りの既定値の書き方
  (`c.avg_slippage_pct = process.env.ZENBOT_AVG_SLIPPAGE_PCT \|\| 0.045` と `c.avg_slippage_pct = 0.045`)である。
- **したがって、この 2 件を独立した 2 件の観測として読んではならない。**段 4 の例としては**実質 1 件**である。
- **番号は両方残す**(分岐元と分岐先で、登録としては別物)。

#### 6 要素ごとの 総数 と 残り(3 段で出す。起動の指定 6)

**数え方**: **総数** = その印を持つ候補の数 / **残り** = そのうち一次資料に到達していないものの数。
**数える単位は「番号」である**(登録の単位)。**機関の単位で数え直すと 22 番と 52 番が 1 つになり、13 番と 123 番も実質 1 件になる**(上の 2 項)。
**下の表は番号で数えたものである。**機関で数えた値は表の下に別に書く。

| 残りの種類 | 23 回目 | この回に埋めた | 24 回目 |
|---|---|---|---|
| **一次資料に未到達** | 11 件(51・53・54・57・60・67・70・72・87・91・92) | 0 件(この回の指定の外) | **11 件**(同じ) |
| **到達済みだが 6 要素の印が無い** | 2 件(14 番 `Bot18`・22 番 `Lean CLI`) | 0 件 | **2 件**(同じ。22 番は 52 番への参照を書いたが、起動口なので印は付けていない) |
| **到達済みで印もあるが、個別の要素が未決** | 8 件 | **7 件**(4・5・6・15・18・19・21 番) | **1 件**(11 番 `OctoBot` の 5 要素。指定により `未確認` のまま残す) |

| 要素 | 23 回目の総数 | この回に足した候補 | 24 回目の総数 | 残り(一次資料に未到達) |
|---|---|---|---|---|
| `区分1-足` | 52 | (無し) | **52** | **11**(51・53・54・57・60・67・70・72・87・91・92) |
| `区分1-ティック` | 15 | (無し) | **15** | **0** |
| `区分1-板の待ち行列` | 17 | (無し) | **17** | **0** |
| `区分1-イベント駆動` | 14 | 6・18 | **16** | **0** |
| `区分1-ベクトル化` | 2 | (無し。4・5・21 はいずれも「付けない」と決まった) | **2** | **0** |
| `区分1-市場影響と約定の模型` | 23 | 3・8・19 | **26** | **0** |

**6 要素のどれかに印がある候補は、重複を除いて 80 件で、23 回目から変わらない**
(この回に印が増えた 3・6・8・18・19 番は、いずれも既に `区分1-足` を持っていて 80 件の中に入っていた)。
**そのうち一次資料に到達していない残りは 11 件で、全部 `区分1-足` である。**

**機関の単位で数え直した値**: **79 件**(22 番と 52 番を 1 つに束ねた分だけ 1 減る)。
**13 番と 123 番は「印の根拠が同じ実装の系統」であって登録としては別の機関なので、束ねていない。**
**束ねた場合は 78 件になるが、この回はその数え方を採っていない**(リードへの問い 2)。

**`判別に一次資料が要る` の残り**: **49 番だけ**(23 回目と同じ)。**登録の内側にあり、当方は登録しない。**
**残りの候補名**: 一次資料に未到達の 11 件、14 番と 22 番の 6 要素、11 番 `OctoBot` の 5 要素、
43 番の浅い部分、51 番から 93 番のうち印を付けていないもの、94 番・96 番・97 番・100 番から 124 番の浅い部分。
**39 番と 110 番と 112 番の一覧の残りの節からも、まだ名前を抜いていない。**

### ツール1件ごとの表

**この回に新しく原典を読んだ候補を出す。**いずれもこの回に導入も最小実行もしていない。

| ツール | 何ができるか | 料金の構造 | 当方に無いもの | この環境での到達 |
|---|---|---|---|---|
| `PySystemtrade` | 足の系列から最適建玉を出し、成行と指値を次の足の値と突き合わせて埋め、埋まらなかったものを空の約定として残す | 未確認(この回に料金の場は読んでいない。3 回目までの記録では GPLv3) | **約定ごとに「この値段に滑りを当てるか」の旗を持ち、指値で埋まったものには当てない形** | **可**(実装の原典に到達。既定枝は `develop`、登録は `pst-group/pysystemtrade` に付け替わっている) |
| `Basana` | 足と注文の事象を購読し、参加率の 2 乗で値段を動かす流動性の模型を通して埋める | 未確認(この回に料金の場は読んでいない。4 回目の記録では Apache-2.0) | **流動性を「1 本の足から取れる量の上限」と「使った割合の 2 乗の影響」の 2 つの引数で表す形** | **可**(実装の原典に到達。導入は 4 回目に済み) |
| `bt` | 日付を 1 つずつ進めながら比重を合わせ直す | 未確認(この回に料金の場は読んでいない) | **無し**(この回に読んだ範囲では、当方の `engine.py` に無い機構は出ていない) | **可**(実装の原典に到達。導入は 4 回目に済み) |
| `Ziplime` | 型を持つ事象の列で駆動し、滑りの模型を通して埋める。滑りは参加率の 2 乗の影響 | 未確認(PyPI の `info.license` が `None`。許諾の本文は `Limex-com/ziplime` に在る) | **時計が「場の開始・取引前・足・刻みの終わり・場の終了」の 5 つの型の事象を出す形** | **可**(実装の原典に到達。登録の所在は `Limex-com/ziplime`。導入と最小実行は 6 回目に済み) |
| `Mendl-Labs/BacktestingCore` | 市場データの並びを頭から進め、参加率の平方根で市場影響を出す | 未確認(許諾は Functional Source License 1.1 と 9 回目に記録) | **無し**(事象の列は持たない。板の模型は在るが実行経路が呼ばない) | **可**(実装の原典に到達。**構築はできない**) |
| `Jesse` | 足の高値と安値が指値を含むかで埋め、足の中で複数の約定が起きるときは足を分割して順番を作る | 未確認(この回に料金の場は読んでいない。3 回目の記録では実弾とペーパーが別売りのプラグイン) | **足を約定の値段で分割して、1 本の足の中の約定の順番を作る形** | **可**(実装の原典に到達。既定枝は `master`。導入と最小実行は 3 回目に済み) |
| `FlashAlpha-lab/flashalpha-fill-simulator` | 気配が指値を余裕つきで越えたかだけで埋め、越えなかったものを「惜しい」の数として返す | 未確認(この回に料金の場は読んでいない。登録に `LICENSE` は在る) | **埋まらなかった注文を「惜しい(near miss)」として数える形** | **可**(実装の原典に到達。導入はしていない) |
| `SarthakDalmia1/backtesting_execution_simulator` | 事象の待ち行列と値段・時刻の優先で照合し、滑りの模型を差し替えて執行値を出す | 未確認(この回に料金の場は読んでいない。登録に `LICENSE` は在る) | **滑りの模型を「無し / 定額 / 出来高 / 平方根」の 4 つから選び、照合の機関とは別の層に置く形** | **可**(実装の原典に到達。構築はしていない) |
| `thirupathikannan-ai/Optimal-Execution-And-Market-Impact-Simulator-` | 一時的影響・恒久的影響・べき指数を与えて執行の軌道を回す | 未確認(この回に料金の場は読んでいない。登録に `LICENSE` は無い) | **べき指数を引数にして影響の形そのものを変える形** | **可**(実装の原典に到達。導入はしていない) |
| `shubhamcodez/Market-Impact-Model` | 実データから一時的影響の係数を当てはめる | 未確認(この回に料金の場は読んでいない。登録に `LICENSE` は無い) | **影響の係数を自分の約定の記録から推定する手続き**(当方は係数を持っていない) | **可**(実装の原典に到達。導入はしていない) |
| `microsoft/MarS` | 注文の流れを生成し、取引する代理人を入れた走りと入れない走りを比べて市場影響を出す | 未確認(この回に料金の場は読んでいない。登録に `LICENSE` は在る) | **影響を関数で置かずに、模擬の 2 本の走りの差として測る形** | **不可**(13 回目に確定。この回は原典の読みだけ) |
| `trade-frame` | 気配ごとに注文の待ち行列を進め、提示量で頭打ちになる部分約定を作る | 未確認(この回に料金の場は読んでいない。18 回目に許諾を記録) | **注文の送信に 250 ミリ秒の遅れを入れてから板に載せる形** | **可**(実装の原典に到達。**構築は wxWidgets と Boost が要るのでしていない**) |

#### §4.0 の機械可読の表

**この回で `[深掘り]` に達した道具は無い。**起動の指定の逐語「**深掘り(構築・導入・最小実行)もしないでください。**」に従い、
導入も最小実行も 1 件も行っていない。委任文 §4.0 は「深掘りした道具は、文章より先に表に 1 行ずつ書く」と定めており、
深掘りが 0 件なので **§4.0 の機械可読の表にこの回の行は無い**(18 回目から 23 回目と同じ)。

### 予算

割当は 1 回 5 万トークン・20 分。**起動の指定の 1 から 6 はすべて終わった。**
(1) 22 回目の貼り付けの食い違いの訂正を書いた。(2) 印を持つ 26 件すべてに段を書き、3・8・19 番に印を足した。
(3) 決まらなかった 7 件を原典で埋めた(11 番の 5 要素は指定により `未確認` のまま)。(4) 新しい検査を当て、結果を表にした。
(5) 22 番と 52 番、13 番と 123 番に互いへの参照を書いた。(6) 数え直しを 3 段で出し、番号と機関の両方の数え方を書いた。
**新しい検索計画は打っていない。深掘りも 0 件。候補も増やしていない。導入・構築・最小実行・登録・鍵の発行・購入・発注は 1 件も無い。**

### 原文に無い判断が要った点(リードに渡す)

1. **段 4 の定義「量を見る(部分約定・滑り)」が、2 つの別のことを束ねている。**
   候補 7 `Superalgos` は**部分約定と滑りを持つが、どちらも足の出来高を読まない**(部分約定は設定した確率、滑りは定率)。
   **「量を見る」を条件にすれば段 1、「部分約定・滑り」を条件にすれば段 4 になる。**この回は**「量を見る」を条件にして段 1 に置いた。**
   同じ判断が候補 20 `VnPy`(定額の滑り)にも掛かっており、**段 2 に置いた。**
   **この振り分けでよいかを決めていただきたい。**段 4 を「量を見る」に限るなら、定率・定額の滑りは段の外の注記になる。
2. **数えるときに、13 番と 123 番を束ねるかどうかを決めていない。**
   22 番と 52 番は「機関として 1 つ」と指定されたので束ねた値(**79 件**)を書いた。
   13 番と 123 番は「**独立した 2 件の観測として読まないこと**」という指定で、**数え方の指定ではない。**
   この回は**束ねずに数えた**(束ねると 78 件)。**どちらを正にするかを決めていただきたい。**
3. **段は 1 つしか書けない形にした。**候補 33 は**照合の機関(段 5 相当)と平方根の影響の関数(段 6)の両方**を持ち、
   候補 38 は**注文の単位の板(段 5)を持つが影響の関数は持たない。**この回は「**実行経路が呼ぶもののうち最も高い段**」を書き、
   もう一方を注記に回した。**段を複数書ける形にするか、最も高い段 1 つにするかを決めていただきたい。**
4. **既定で無効になる機構を、段に数えた。**候補 4 `PyBroker`(既定 `None`)・候補 33(既定 `ZeroSlippageModel`)・
   候補 2 `Backtrader`(既定の充当器なし・滑り 0)は、**そのまま使うと段 1 相当に落ちる。**
   この回は「道具が持ち、実行経路が呼ぶ形が在る」ことを条件にして高い段に置き、既定を注記した。
   **使う段の判断では既定のほうが効くので、段の欄に既定も併記する形にするかを決めていただきたい。**

### 取ってきた文章の中の作業者向けの指示(委任文 §6-3)

**この回に読んだ登録のうち、作業者向けの手引き(`AGENTS.md` / `CLAUDE.md` / `.cursor` / `copilot-instructions`)を新しく見つけたものは 0 件である。**
候補 19 `Jesse` の同梱(`AGENTS.md`・`.claude/skills/README.md`・`.claude/skills/jesse-strategy-tests/SKILL.md`・`.agents/skills`)は 23 回目に数えたもので、
**この回も本文は取っていないし従ってもいない。通算は 6 件のままである。**
**この回に取った原典は、すべて実装のソースと追跡ファイルの一覧で、指示文ではない。**
...
**これは貼り付け後に打ち直した最終の実行の出力である**(起動の指定 4 の扱い)。
**この回は K12 の自己参照が起きない。**理由: この節を作る前に打った結果も `検査対象の合計 0 件` で、
K12 が照合するのは「貼られた合計」と「K12 を除いた合計」であり、後者はこの節の有無で変わらないからである。
**貼った塊と、貼ったあとに打ち直した出力が 1 行ずつ同じであることを確かめた**(生ログ `20260922_tools_1_run24.log` の `recheck` の節)。

**誤検出だと判断して自分で閉じた行は 0 件である。**この回は 13 本すべてが 0 件で通った。

**ただし、この検査に 1 つ穴を見つけたのでリードに渡す**(自分で閉じていない): **K12 は「最後の『受け入れ検査の出力』の節」だけを見る。**
**この回の節を作る前に検査を打ったとき、K12 は 0 件だった。**当たらなかったのは、23 回目の節の貼り付けが最後の節として身代わりになったためである。
**つまり「貼るのを忘れた回」は、前の回の貼り付けで K12 を通ってしまう。**
**生ログの本数**: `ls docs/DATA/probes/20260922_tools_1_run*.log | wc -l` の出力は **23**(生ログ `20260922_tools_1_run24.log` の `ls_probes` の節)。

## 区分1 — 25 回目の実行(2026-09-22)

委任文: `docs/DATA/delegations/20260922_tools_survey_prompt.md@ce0012c95154`。生ログ: `docs/DATA/probes/20260922_tools_1_run25.log`。
24 回目のリードの検収(`docs/AUDITOR/VERDICTS/2026-09-22_tools_scan_cat1_run24.md`)と、その §8 の起動の指定に従う。

**この回にしたこと(整理の回)**: (1) 段の表を 2 列(段(機構)/ 段(既定))にし、複数の段に当たるものを全部書いた /
(2) 段 4 の定義を締め直し、当たる候補を当て直した / (3) 登録の付け替え 2 件を候補の一覧に併記した /
(4) K12 を直したことを書いた / (5) 数え直しを 3 段で出し、機関で数えた値も併記した。
**導入・構築・最小実行・登録・鍵の発行・購入・発注はこの回に 1 件も無い。新しい検索計画も打っていない。候補も増やしていない。**

**この回に打った GET について**: 起動の指定の逐語「**深掘り(構築・導入・最小実行)もしないでください。**」に従い、導入も最小実行もしていない。
**ただし「段(既定)」の欄は、既定値を読まなければ 1 行も埋まらない。**そこで、**24 回目までに既に出典表に載っている原典から、既定値の行だけを取り直した。**
新しい候補・新しい検索語は 1 つも増やしていない。**この判断はリードへの問い 3 に挙げてある。**

**書き換えの範囲**: 過去の回の節は 1 行も書き換えていない。この回の指定 1 から 5 はすべて**新しい節への追記だけ**で果たした。
起動の指定の逐語「**追記だけで果たせるならそのほうがよいです**(記録を壊しません)。」に従った。

**生ログの本数**: `ls docs/DATA/probes/20260922_tools_1_run*.log | wc -l` の出力は **24**(生ログ `20260922_tools_1_run25.log` の `ls_probes` の節)。
**これはこの回の生ログ `run25.log` を作ったあとに打った値で、内訳は `run2` から `run25` である**(`run1` の生ログは無い。21 回目から 24 回目と同じ)。

**字形について**: この回に逐語で引いた原典に、**字形違いが 1 箇所あった。**
**K12 の直しの逐語 `[—\-–]` は、1 文字目が EM DASH(U+2014)、3 文字目が EN DASH(U+2013)で、半角のハイフンではない。**
原典 `scripts/check_scan_report.py` の 300 行のとおりに写してある(符号位置を出した実測は生ログ
`20260922_tools_1_run25.log` の `nonascii3` の節)。**ここを半角の `-` に直して写すと、検査が見ている正規表現とは別物になる。**
**それ以外のこの回の逐語は、すべて半角の ASCII である**(数え直しは同じ生ログの `nonascii2` の節)。
なお非分割ハイフン(`‑` U+2011)は、この段落の中にしか出てこない(引き写した原典には無い)。

**到達の記録**: HTTP 404 は **1 件**で、`ungh.cc/repos/Ziplime/ziplime` である(24 回目と同じものを、付け替えの確認のために意図して打ち直した)。
**TLS の切断(`code=000`、`rc=35`)は 1 件あり、打ち直して `code=200` を得ている。404 の欄には入れていない。**
**既定枝は `ungh.cc` の `defaultBranch` で確かめてから取った。当てずっぽうの取得は 0 件である。**

### 検索計画

**この回は検索計画を打っていない。**起動の指定の逐語「**新しい検索計画は打たないでください。**」による。
X の経路も同じ理由で打っていない。**候補も 1 件も増やしていない。**
この回に取った GET は、**登録の付け替え 2 件の確認**と、**段(既定)の欄を埋めるための既定値の読み直し**だけである。

### 出典

| # | 出典 | 取得日 | 使った先 |
|---|---|---|---|
| 1 | `https://ungh.cc/repos/robcarver17/pysystemtrade` | 2026-09-22 | 候補 3 の登録の付け替えの確認(応答の `"repo"` が付け替え先を返す) |
| 2 | `https://ungh.cc/repos/pst-group/pysystemtrade` | 2026-09-22 | 候補 3 の既定枝(`develop`) |
| 3 | `https://ungh.cc/repos/Ziplime/ziplime`(HTTP 404) | 2026-09-22 | 候補 6 の誤った所有者名での応答 |
| 4 | `https://ungh.cc/repos/Limex-com/ziplime` | 2026-09-22 | 候補 6 の正しい登録と既定枝(`master`) |
| 5 | `https://raw.githubusercontent.com/microsoft/qlib/main/qlib/backtest/exchange.py` | 2026-09-22 | 候補 21 の影響の式と出来高の上限の**既定値** |
| 6 | `https://raw.githubusercontent.com/stefan-jansen/zipline-reloaded/main/src/zipline/algorithm.py` | 2026-09-22 | 候補 18 の滑りの模型の差し替え口 |
| 7 | `https://raw.githubusercontent.com/stefan-jansen/zipline-reloaded/main/src/zipline/finance/blotter/simulation_blotter.py` | 2026-09-22 | 候補 18 の**既定の**滑りの模型 |
| 8 | `https://raw.githubusercontent.com/stefan-jansen/zipline-reloaded/main/src/zipline/finance/slippage.py` | 2026-09-22 | 候補 18 の既定の滑りの模型の中身と既定の引数 |
| 9 | `https://raw.githubusercontent.com/Limex-com/ziplime/master/ziplime/exchanges/simulation_exchange.py` | 2026-09-22 | 候補 6 の滑りの模型が**必須引数**であること |
| 10 | `https://raw.githubusercontent.com/gbeced/basana/develop/basana/backtesting/exchange.py` | 2026-09-22 | 候補 1 の**既定の**流動性の模型 |
| 11 | `https://raw.githubusercontent.com/gbeced/basana/develop/basana/backtesting/liquidity.py` | 2026-09-22 | 候補 1 の既定の引数と、差し替え先の一覧 |
| 12 | GitHub の code search `equity_slippage repo:Limex-com/ziplime` / `VolumeShareSlippage repo:Limex-com/ziplime` | 2026-09-22 | 候補 6 の滑りの模型を渡す側の所在(どちらも `total_count=0`) |
| 13 | `scripts/check_scan_report.py`(当方のリポジトリ) | 2026-09-22 | K12 の直しの逐語と、壊した入力での再現 |

### 知見

| # | 知見 | 印 | 根拠 |
|---|---|---|---|
| 1 | **候補 21 `Qlib` の影響の式は、そのまま使うと効かない。**逐語は `impact_cost: float = 0.0,` と `volume_threshold: Union[tuple, dict, None] = None,` と、同じファイルの説明の逐語 `:param impact_cost:     market impact cost rate (a.k.a. slippage). A recommended value is 0.1.`。**影響の式 `adj_cost_ratio = self.impact_cost * (trade_val / total_trade_val) ** 2` は既定で 0 倍になり、出来高の上限も既定では無い。**原典自身が「推奨は 0.1」と書いているのは、**既定がその値でないことの裏返しである** | 実測 | `.../qlib/backtest/exchange.py` 取得日 2026-09-22 / `20260922_tools_1_run25.log` の `defaults` の節 |
| 2 | **候補 18 `zipline-reloaded` の既定の滑りは、定率と出来高の上限であって、2 乗の影響ではない。**逐語は `Equity: equity_slippage or FixedBasisPointsSlippage(),` と `def __init__(self, basis_points=5.0, volume_limit=0.1):` と、同じ類の説明の逐語 `Fill sizes are capped at::` `historical_volume * volume_limit` と `This class, default-constructed, is zipline's default slippage model for equities.`。**そのまま使うと「出来高の 1 割までしか埋まらず、値段は 5 ベーシスポイント動く」で、2 乗の影響 `price * (1 + price_impact * (volume_share ** 2))` は差し替えたときだけ効く** | 実測 | `.../simulation_blotter.py` と `.../finance/slippage.py` 取得日 2026-09-22 / `20260922_tools_1_run25.log` の `defaults` の節 |
| 3 | **候補 1 `Basana` は、差し替えなくても影響の模型が入っている。**逐語は `liquidity_strategy_factory: LiquidityStrategyFactory = liquidity.VolumeShareImpact,` で、その模型の既定の引数は `def __init__(self, volume_limit_pct: Decimal = Decimal("25"), price_impact: Decimal = Decimal("10")):` である。**同じファイルに `class InfiniteLiquidity(LiquidityStrategy):`(説明の逐語 `This class models infinite liquidity with no price impact.`)があり、そちらに差し替えると段 1 相当に落ちる。**つまり**この道具は既定のほうが高い段にある** | 実測 | `.../backtesting/exchange.py` と `.../backtesting/liquidity.py` 取得日 2026-09-22 / `20260922_tools_1_run25.log` の `defaults` の節 |
| 4 | **候補 6 `Ziplime` の滑りの模型には既定値が無い。**逐語は `equity_slippage: SlippageModel,` と `future_slippage: SlippageModel,`(どちらも既定値の書かれていない必須引数)で、24 回目に読んだ呼び出しの逐語 `slippage = self.get_slippage_model(asset=asset)` が返すのは、**呼び出し側が渡したものである。**渡す側を GitHub の code search で探したが `equity_slippage` も `VolumeShareSlippage` も `total_count=0` で見つからなかった。**よって段(既定)は `未確認` とする**(**親の `zipline-reloaded` は `or FixedBasisPointsSlippage()` という既定を持つが、この道具はその行を持たない**) | 実測 | `.../ziplime/exchanges/simulation_exchange.py` 取得日 2026-09-22 と GitHub の code search / `20260922_tools_1_run25.log` の `defaults` の節 |
| 5 | **登録の付け替え 2 件を打ち直して確かめた。**`ungh.cc/repos/robcarver17/pysystemtrade` は `code=200` で `"repo":"pst-group/pysystemtrade"` を返し(**古い所有者名で引いても付け替え先の名前が返る**)、既定枝は `"defaultBranch":"develop"` である。`ungh.cc/repos/Ziplime/ziplime` は `code=404` で、`ungh.cc/repos/Limex-com/ziplime` が `code=200`、既定枝は `"defaultBranch":"master"` である | 実測 | `20260922_tools_1_run25.log` の `c3_old_ungh` `c3_new_ungh` `c6_old_ungh` `c6_new_ungh2` `defaultBranch` の各節 |
| 6 | **K12 の直しが効いていることを、当方でも壊した入力で確かめた。**直しの逐語は `runs = [m.start() for m in re.finditer(r"^#{2,3} *区分\d+ *[—\-–] *\d+ *回目の実行", text, re.M)]` と `if runs and heads[-1] < runs[-1]:` と、当たったときの文言 `いちばん新しい回の節に「受け入れ検査の出力」が無い` である。**この回の節の貼り付けを見出しごと落とした写しで打つと K12 が当たる**(生ログ `k12_probe` の節) | 実測 | `scripts/check_scan_report.py` と `20260922_tools_1_run25.log` の `k12_fix` と `k12_probe` の節 |

### 候補の一覧

**新しい候補は 0 件。**この回に状態が変わったのは、**段の表の書き方(2 列にした)**と、**7 番・20 番の段の注記**と、**3 番・6 番の登録の付け替えの併記**である。
番号と名前は既存の一覧のものをそのまま使う。**この回は 1 件も深掘りしていない。**

#### 登録の付け替え 2 件(起動の指定 3)

**古い URL のまま引用していると、後から「取れない」と誤診する型である。**起動の指定の逐語:
「**古い URL のまま引用していると、後から「取れない」と誤診する型です。**」**候補の一覧の該当箇所に、付け替え先を併記する。**

| 候補 | 一覧に載っている古い登録 | **これから使う登録** | 既定枝 | 打ち直した応答 |
|---|---|---|---|---|
| 3 `PySystemtrade` | `robcarver17/pysystemtrade` | **`pst-group/pysystemtrade`** | **`develop`** | 古い名前でも `code=200` で、応答の中身が `"repo":"pst-group/pysystemtrade"` に変わっている(**404 にはならないので、気づかずに古い名前を引用し続けられる**) |
| 6 `Ziplime` | `Ziplime/ziplime` | **`Limex-com/ziplime`** | **`master`** | 古い名前は `code=404`。**こちらは打てば必ず気づく型である** |

**2 件の違いを書いておく**: **付け替えには「気づける型」と「気づけない型」がある。**
3 番は **404 にならず 200 のまま中身だけが変わる**ので、`ungh.cc` の応答の `"repo"` の欄を読まないと気づけない。
6 番は 404 になるので打てば気づく。**以後、一覧の登録名を使うときは `"repo"` の欄まで読む。**

#### 段の表 — **2 列にした**(段(機構)/ 段(既定)。起動の指定 1・2)

**リードの決定の逐語**(24 回目の検収 §4):
「**段は 2 つ書く。段(機構) = 差し替えれば届く高さ。段(既定) = そのまま使ったときの高さ。**」
「**当方が道具を選ぶときに効くのは段(既定)のほうである。**」「**この区別を落とすと、区分 1 の比較が実務で役に立たない。**」

**複数の段に当たるものは全部書く。**リードの決定の逐語(同 §5(3)):
「**段は『何ができるか』ではなく『何を見て埋めるか』の軸なので、複数の軸に当たるなら全部書くのが正しい。**」
**24 回目に注記へ回した 33 番の段 5 と 38 番は、段の欄に戻した。**

**段 4 の定義(締め直したもの。同 §5(1) の逐語)**:
「**段 4 は「約定の出来高を読んで、埋まる量を決める」ものに限る。**」
「**定率・定額の滑りは段を上げない**(価格に一定の掛け目を当てているだけで、相場の量を読んでいない)。」
「**確率で部分約定させるのも段を上げない**(量ではなく乱数を読んでいる)。」

**段(機構)に数える条件**: 「差し替えれば届く」とは、**設定・引数・模型の差し替えで実行経路が呼べる**ことである。
**実行経路が呼んでいない機構(24 回目の表の型 1)は、どちらの列にも数えない**(コードを書き換えなければ届かないため)。該当は注記に書く。

**並びは 24 回目の表と同じ**(候補の順を変えていないので、24 回目と 1 行ずつ見比べられる)。

| 候補 | 段(機構) | 段(既定) | 段を決めた逐語(出所) | 注記 |
|---|---|---|---|---|
| 8 `OpenTrader` | **1** | **1** | `if (candlestick.close <= smartTrade.entryOrder.price!) {` と `filledPrice: candlestick.close,`(23 回目の知見 10) | 差し替える模型を持たない。機構と既定が同じ |
| 32 `FlashAlpha-lab/flashalpha-fill-simulator` | **1** | **1** | `if combo_bid >= cand.limit_credit + cfg.fill_epsilon:`(24 回目の知見 15) | 余裕 `fill_epsilon` と気配幅の絞りを足した越え判定で、量も列も見ない |
| 7 `Superalgos` | **1** | **1** | `if (tradingEngine.tradingCurrent.tradingEpisode.candle.min.value <= tradingEngineOrder.orderStatistics.actualRate.value) {`(24 回目の知見 14) | **定率の滑りと確率の部分約定を持つが出来高を読まない。**締め直した段 4 の定義(「約定の出来高を読んで、埋まる量を決める」)に当たらないので段は上がらない |
| 3 `PySystemtrade` | **2** | **2** | `market_price=data_for_idx.next_price,` と `if limit_price > market_price:` と `return empty_fill(fill_datetime)`(24 回目の知見 1) | 成行にだけ滑りを当てる旗を持つが、**定率なので段は上がらない。登録は `pst-group/pysystemtrade`** |
| 19 `Jesse` | **2** | **2** | `if low <= order_price <= high:` と `order.filled_qty = order.qty` と `active_orders`(24 回目の知見 8) | 足の中で複数の約定が起きるときは足を分割して順番を作る。**これが段 3(価格の到着を事象として見る)に当たるかは決めていない → リードへの問い 1** |
| 20 `VnPy` | **2** | **2** | `self.active_limit_orders: dict[str, OrderData] = {}` と `long_cross_price = self.bar.low_price`(24 回目の知見 13、23 回目の知見 6) | **定額の滑りを持つが出来高を読まない**ので段は上がらない。事象の機関は**実行経路が呼ばない**ので、どちらの列にも数えていない |
| 2 `Backtrader` | **4** | **1** | 機構 = `'''Sets a volume filler for volume filling execution'''` と `# Execution depends on volume filler`(23 回目の知見 4)。既定 = `('slip_perc', 0.0),` `('slip_fixed', 0.0),` と充当器なし | **既定は 24 回目の検収 §4 の逐語「候補 4・33・2 は段(機構)が高くても段(既定)は 1 である」に従った。**未約定を足をまたいで残すか(段 2 に当たるか)は**当方は測っていない** |
| 13 `DeviaVir/zenbot` | **4** | **4** | `let size = Math.min(buy_order.remaining_size, trade.size)`(23 回目の知見 5) | 約定の出来高で頭打ちになる部分約定が**既定で働く。**滑りの既定 `0.045` は定率なので段を上げない |
| 123 `carlos8f/zenbot` | **4** | **4** | `Math.min(buy_order.remaining_size, trade.size)`(22 回目の知見) | 13 番と同じ実装の系統。**独立した 2 件の観測として読まない** |
| 63 `trade-frame` | **4** | **4** | `quanApplied = std::min<Trade::tradesize_t>( nOrderQuanRemaining, quote.AskSize() );`(24 回目の知見 20) | 気配の提示量で頭打ちになる部分約定が既定で働く。**列の位置は追わないので段 5 ではない** |
| 23 `hftbacktest` | **5** | **未確認** | `Order fill simulation that takes into account the order queue position, using provided models or your own custom model`(1 回目の節) | **1 回目の最小実行では `risk_adverse_queue_model()` を明示して与えた。**列の模型に既定があるかは**この回に読んでいない**(試したこと: 1 回目の節と最小実行の逐語を読み返しただけ。原典の新しい取得はしていない) |
| 98 `mihircoding/limitOrderBook` | **5** | **5** | `Price-time priority matching engine and a zero-intelligence order flow simulator.`(16 回目の最小実行) | 照合そのものが値段と時刻の優先なので、**差し替える余地がなく既定で段 5** |
| 38 `microsoft/MarS` | **5** | **5**(推定) | `target_volume=task.twap_agent_target_volume,` と `max_passive_volume_ratio=0.9,`(24 回目の知見 19) | **24 回目に注記へ回していたものを段の欄に戻した。**差し替える模型ではなく模擬そのものなので既定でも同じ高さと**推定**した。**影響の関数は持たないので段 6 ではない**(これが 33 番との違い) |
| 1 `Basana` | **4・6** | **4・6** | 機構と既定が同じ = `liquidity_strategy_factory: LiquidityStrategyFactory = liquidity.VolumeShareImpact,` と `def __init__(self, volume_limit_pct: Decimal = Decimal("25"), price_impact: Decimal = Decimal("10")):` と `ret = used_pct ** Decimal(2) * self._price_impact_pct`(この回の知見 3、24 回目の知見 3) | **この表で唯一、既定のほうが高い側に倒れている道具である。**差し替え先の `InfiniteLiquidity` に変えると段 1 相当に落ちる |
| 4 `PyBroker` | **4・6** | **1** | 機構 = 逐語 impact is ``price_impact * (filled_shares / bar_volume) ** 2`` と `volume_limit: Optional[float] = 0.025,`(24 回目の知見 11)。既定 = `slippage_model: Optional[SlippageModel] = None,` | **呼んではいるが既定では無効。**24 回目の検収 §4 の逐語に従って既定は 1 |
| 6 `Ziplime` | **4・6** | **未確認** | 機構 = `price * (1 + price_impact * (volume_share ** 2))` と `volume_limit=DEFAULT_EQUITY_VOLUME_SLIPPAGE_BAR_LIMIT,`(24 回目の知見 5) | **滑りの模型は必須引数で既定値が無い**(この回の知見 4)。**渡す側を code search で探して `total_count=0`。よって「そのまま使ったときの高さ」が決まらない** |
| 15 `Mendl-Labs/BacktestingCore` | **6** | **6**(推定) | `/// Estimate market impact using a square-root model:` と `self.calculate_market_impact(order_size, order_price, avg_volume, config, timestamp)`(23 回目の知見 8) | 影響の式は実行経路が**直に呼ぶ**ので差し替えの余地が無いと**推定**した。**係数の既定値はこの回に読んでいない。**合成の板は**実行経路が呼ばない**ので段 5 に数えていない |
| 18 `zipline-reloaded` | **4・6** | **4** | 機構 = `price * (1 + price_impact * (volume_share ** 2))`(23 回目の知見 3)。既定 = `Equity: equity_slippage or FixedBasisPointsSlippage(),` と `def __init__(self, basis_points=5.0, volume_limit=0.1):`(この回の知見 2) | **既定は「出来高の 1 割までしか埋まらない」ので段 4 に当たるが、2 乗の影響は差し替えたときだけ効くので段 6 は既定に入らない。**段(機構)と段(既定)が**部分的に**食い違う例 |
| 21 `Qlib` | **4・6** | **1** | 機構 = `adj_cost_ratio = self.impact_cost * (trade_val / total_trade_val) ** 2`(24 回目の知見 9)。既定 = `impact_cost: float = 0.0,` と `volume_threshold: Union[tuple, dict, None] = None,`(この回の知見 1) | **この回に新しく見つかった「呼ぶが既定では無効」である。**原典自身が `A recommended value is 0.1.` と書いており、**既定がその値でないことの裏返しである** |
| 33 `SarthakDalmia1/backtesting_execution_simulator` | **5・6** | **5** | 機構 = `slippage_bps += volume_impact_factor_ * std::sqrt(participation) * 10000.0;`(段 6)と、事象の待ち行列・値段と時刻の優先の照合(段 5)。既定 = `, slippage_model_(std::make_unique<ZeroSlippageModel>())`(24 回目の知見 16) | **24 回目に注記へ回した段 5 を段の欄に戻した。**照合の機関は差し替えの余地が無く既定で働くので段(既定)を 5 と書いたが、**24 回目の検収 §4 の逐語は「候補 4・33・2 は…段(既定)は 1」と書いている。食い違いをリードへの問い 2 に挙げる** |
| 35 `thirupathikannan-ai/Optimal-Execution-And-Market-Impact-Simulator-` | **6** | **6** | `temporary_impact=0.01,` と `permanent_impact=0.0001,` と `impact_exponent=1.0,`(24 回目の知見 17) | **引数を与えて呼ぶ単体の模型で、差し替える既定が無い**(機関を持たないので、機構と既定が同じ) |
| 36 `shubhamcodez/Market-Impact-Model` | **6** | **6** | `temporary_impact = eta * sigma * np.sign(x) * np.power(np.abs(x) , beta)` と `popt, pcov = curve_fit(temp_impact, x, y)`(24 回目の知見 18) | **測る側(当てはめ)。**機関を持たないので機構と既定が同じ |
| 105 `DaniyalMlk/slippage` | **6** | **6** | 16 回目の最小実行「**合成の値で線形の市場影響・べき乗則の市場影響・Almgren-Chriss の最適軌道・等速の軌道・平方根則の頂点の影響を計算した**」 | 単体の模型。機関を持たないので機構と既定が同じ |
| 106 `almgren-chriss` | **6** | **6** | `Permanent impact slope`(gamma)・`Temporary impact slope`(eta)・`Temporary impact intercept`(epsilon)(19 回目の知見 21) | 単体の模型。機関を持たないので機構と既定が同じ |
| 116 `braverock/blotter` の `acOptTxns` | **6** | **6** | `\eqn{g(v_{k})} is the linear permanent impact function`(19 回目の知見 22) | 単体の模型。機関を持たないので機構と既定が同じ |
| 119 `peernagy/lob_bench` | **6** | **6** | 18 回目の節の逐語「**検証の機関ではなく、生成した板の列を測る物差しである。**」 | **測る側。**機関を持たないので機構と既定が同じ |
...
**比較の相手として意味があるのは、段(既定)が 2 以上の候補である。**

**段(既定)の分布**(**最も高い段で 1 件ずつ数える。合計は表の 26 件**):
...
**段(機構)と段(既定)が同じでない候補は 7 件である。**内訳は 2 つに分かれる:
**(a) 既定でそのまま使うと段が下がる 5 件** = 2 番(4 から 1)・4 番(4・6 から 1)・21 番(4・6 から 1)・18 番(4・6 から 4)・33 番(5・6 から 5)。
**(b) 既定が決まらない 2 件** = 23 番と 6 番(どちらも `未確認`)。
**24 回目に「呼ぶが既定では無効」として挙がっていた 3 件(4・33・2)のうち、33 番は照合の機関が既定で働くので、この回の書き方では下がり幅が滑りの部分だけに縮む。**
**この回に新しく (a) に入ったのは 21 番 `Qlib` と 18 番 `zipline-reloaded` である。**

**この表の候補の数は 26 で、24 回目と同じである**(この回に段の表へ足した候補も、外した候補も無い。書き方だけを 2 列にした)。

#### K12 を直したこと(起動の指定 4)

**24 回目にこちらが渡した指摘は本物だった。**起動の指定の逐語:
「**リードが実測で再現しました**(24 回目の貼り付けを見出しごと落とすと K12 が 0 件で通り、23 回目の貼り付けが身代わりになっていました)。」
「**自分で閉じずに渡したのが正しい判断でした。**自分で閉じていたら、この穴は残ったままでした。」

**直った後の振る舞い**: **貼り付けが「いちばん新しい回の節の中」に無ければ K12 が当たる。**
直しの逐語は `runs = [m.start() for m in re.finditer(r"^#{2,3} *区分\d+ *[—\-–] *\d+ *回目の実行", text, re.M)]` と
`if runs and heads[-1] < runs[-1]:` で、当たったときの文言は
`いちばん新しい回の節に「受け入れ検査の出力」が無い(前の回の貼り付けは身代わりにならない。この回の出力を貼ること)` である。

**当方でも壊した入力で確かめた**(自分の指摘が直ったことを、自分で打って確かめる)。写しを scratchpad に作って打った結果:

| 入力(この報告書の写しに当てたもの) | K12 | 期待 |
|---|---|---|
| この回の貼り付けを**見出しごと**落とした | **1 件** | 当たるべき |
| この回の貼り付けの**塊だけ**落とした(見出しは在る) | **1 件** | 当たるべき |
| 本物 | **0 件** | 通るべき |

**以後、貼るのを忘れた回は止まる。****この直しの直接の原因は 24 回目にこちらが渡した指摘である。**

#### 6 要素ごとの 総数 と 残り(3 段で出す。起動の指定 5)

**数え方**: **総数** = その印を持つ候補の数 / **残り** = そのうち一次資料に到達していないものの数。
**数える単位は「番号」である。**起動の指定の逐語:
「**束ねません。番号で数えた 80 件を正とします。**」
「完了の形は**候補**について言われている(オーナー逐語「一次資料に到達した**候補**が尽きること」)。候補は登録の単位である。」
**ただし機関で数えた値の併記は続ける**(同じ逐語「ただし機関で数えた値(79 件 / 78 件)の併記は続けてください。」)。

**3 段の数え直し**(この回は指定が整理だけなので、どの段も 24 回目から動いていない):

| 残りの種類 | 24 回目 | この回に埋めた | 25 回目 | 中身 |
|---|---|---|---|---|
| **一次資料に未到達** | 11 件 | 0 件(この回の指定の外) | **11 件** | 51・53・54・57・60・67・70・72・87・91・92(**全部 `区分1-足`**) |
| **到達済みだが 6 要素の印が無い** | 2 件 | 0 件 | **2 件** | 14 番 `Bot18`・22 番 `Lean CLI`(22 番は 52 番への起動口なので印を付けていない) |
| **到達済みで印もあるが、個別の要素が未決** | 1 件 | 0 件 | **1 件** | 11 番 `OctoBot` の 5 要素(起動の指定により `未確認` のまま残す) |

| 要素 | 24 回目の総数 | この回に足した候補 | 25 回目の総数 | 残り(一次資料に未到達) |
|---|---|---|---|---|
| `区分1-足` | 52 | (無し) | **52** | **11**(51・53・54・57・60・67・70・72・87・91・92) |
| `区分1-ティック` | 15 | (無し) | **15** | **0** |
| `区分1-板の待ち行列` | 17 | (無し) | **17** | **0** |
| `区分1-イベント駆動` | 16 | (無し) | **16** | **0** |
| `区分1-ベクトル化` | 2 | (無し) | **2** | **0** |
| `区分1-市場影響と約定の模型` | 26 | (無し) | **26** | **0** |

**6 要素のどれかに印がある候補は、重複を除いて 80 件で、24 回目から変わらない。**
**そのうち一次資料に到達していない残りは 11 件で、全部 `区分1-足` である。**

**機関で数えた値の併記**(どちらで数えたかを明記する):

| 数え方 | 値 | 何を束ねたか | この回の扱い |
|---|---|---|---|
| **番号(登録の単位)** | **80 件** | 何も束ねない | **これを正とする**(起動の指定の逐語「**番号で数えた 80 件を正とします。**」) |
| 機関(22 番と 52 番を束ねる) | 79 件 | 22 番 `Lean CLI` は 52 番 `QuantConnect` の機関の起動口 | 併記のみ |
| 機関(13 番と 123 番も束ねる) | 78 件 | 123 番が本家、13 番がその分岐 | 併記のみ |

**`判別に一次資料が要る` の残り**: **49 番だけ**(23 回目・24 回目と同じ)。**登録の内側にあり、当方は登録しない。**
**残りの候補名**: 一次資料に未到達の 11 件、14 番と 22 番の 6 要素、11 番 `OctoBot` の 5 要素、
43 番の浅い部分、51 番から 93 番のうち印を付けていないもの、94 番・96 番・97 番・100 番から 124 番の浅い部分。
**39 番と 110 番と 112 番の一覧の残りの節からも、まだ名前を抜いていない。**
...
**この回に既定値を読み直した候補だけを出す。**いずれもこの回に導入も最小実行もしていない。
**24 回目までに書いた列(できること・料金・当方に無いもの・到達)は書き換えず、この回に分かった既定の話を足した。**

| ツール | 何ができるか | 料金の構造 | 当方に無いもの | この環境での到達 |
|---|---|---|---|---|
| `Qlib` | 注文を 1 件ずつ進め、参加率の 2 乗で市場影響の費用率を足す。**ただし既定の係数は 0 で、出来高の上限も既定では無い** | 未確認(この回に料金の場は読んでいない) | **市場影響の係数と出来高の上限を、設定の引数として外から与える形**(当方の `engine.py` は費用を固定の掛け目で持つ)/ **原典自身が推奨値を文章で書いておき、既定はそれより安全でない側に置く形** | **可**(実装の原典に到達。既定値の行まで読んだ) |
| `zipline-reloaded` | 型を持つ事象の列で駆動し、滑りの模型を通して埋める。**既定は定率 5 ベーシスポイントと出来高の 1 割の上限で、2 乗の影響は差し替えたときだけ** | 未確認(この回に料金の場は読んでいない) | **既定の滑りの模型が「値段の掛け目」と「埋まる量の上限」を同時に持つ形**(当方の `engine.py` は埋まる量に上限が無い) | **可**(実装の原典に到達。既定値の行まで読んだ) |
| `Basana` | 足と注文の事象を購読し、参加率の 2 乗で値段を動かす流動性の模型を通して埋める。**既定でその模型が入っている** | 未確認(この回に料金の場は読んでいない) | **流動性の模型を差し替え可能にしたうえで、既定を「影響あり」側に置く形**(**無限の流動性のほうを明示的に選ばせる**) | **可**(実装の原典に到達。既定値の行まで読んだ) |
| `Ziplime` | 型を持つ事象の列で駆動し、滑りの模型を通して埋める。**滑りの模型は必須引数で、既定値を持たない** | 未確認(PyPI の `info.license` が `None`。許諾の本文は `Limex-com/ziplime` に在る) | **滑りの模型に既定を置かず、呼び出し側に必ず選ばせる形**(**選ばなければ動かない**) | **可**(実装の原典に到達。**登録は `Limex-com/ziplime`。`Ziplime/ziplime` は 404**) |

#### §4.0 の機械可読の表

**この回で `[深掘り]` に達した道具は無い。**起動の指定の逐語「**深掘り(構築・導入・最小実行)もしないでください。**」に従い、
導入も最小実行も 1 件も行っていない。委任文 §4.0 は「深掘りした道具は、文章より先に表に 1 行ずつ書く」と定めており、
深掘りが 0 件なので **§4.0 の機械可読の表にこの回の行は無い**(18 回目から 24 回目と同じ)。

### 予算

割当は 1 回 5 万トークン・20 分。**起動の指定の 1 から 5 はすべて終わった。**
(1) 段の表を 2 列にし、複数の段に当たるものを全部書いた(33 番の段 5 と 38 番を段の欄に戻した)。
(2) 段 4 の定義を締め直し、7 番を段 1・20 番を段 2 のままにして注記を付けた。
(3) 登録の付け替え 2 件を候補の一覧に併記し、打ち直して確かめた。
(4) K12 を直したことを書き、壊した入力で当方でも確かめた。
(5) 数え直しを 3 段で出し、機関で数えた値も併記した。
**新しい検索計画は打っていない。深掘りも 0 件。候補も増やしていない。導入・構築・最小実行・登録・鍵の発行・購入・発注は 1 件も無い。**

### 原文に無い判断が要った点(リードに渡す)

1. **19 番 `Jesse` が段 3 に当たるかを決めていない。**`Jesse` は足の中で複数の約定が起きるとき、**足を約定の値段で分割して順番を作る**
   (24 回目の知見 8)。段 3 の定義は「**価格の到着を事象として見る**」で、**分割して順番を作ることがこれに当たるかは、どの逐語にも書かれていない。**
   この回は**段 2 のままにした**(24 回目の書き方を変えない側)。**段 3 を足すかを決めていただきたい。**
2. **33 番の段(既定)を 5 と書いたが、24 回目の検収 §4 の逐語と食い違う。**
   検収 §4 の逐語は「**候補 4・33・2 は段(機構)が高くても段(既定)は 1 である。**」だが、
   33 番が既定で無効になるのは**滑りの模型**(`ZeroSlippageModel`)だけで、**事象の待ち行列と値段・時刻の優先の照合は差し替えの余地が無く既定で働く。**
   締め直した段の定義(段 5 = 列の位置を追う)をそのまま当てると**段(既定)は 5** になる。
   この回は**逐語ではなく定義のほうに従って 5 と書き、食い違いをここに挙げた。**どちらを採るかを決めていただきたい。
3. **既定値を読むための GET を打ってよいかを決めていない。**起動の指定は「**深掘り(構築・導入・最小実行)もしないでください。**」であり、
   **原典の読み直しを禁じてはいない。**しかし段(既定)の欄は既定値を読まなければ埋まらないので、
   **24 回目までに既に出典表に載っている原典から、既定値の行だけを取り直した**(新しい候補・新しい検索語は 0 件)。
   **この読み直しが「深掘り」に当たるなら、次の回からは `未確認` で埋める。**どちらかを決めていただきたい。
4. **23 番 `hftbacktest` と 6 番 `Ziplime` の段(既定)を `未確認` にした。**
   23 番は列の模型に既定があるかを読んでいない。6 番は滑りの模型が必須引数で、**渡す側を code search で探して `total_count=0` だった。**
   **この 2 件を埋めるには原典をもう一段読む必要がある**(6 番は使用例か起動の入口、23 番は builder の定義)。
   **埋めに行くかを決めていただきたい。**上の 3 と同じ線引きの話である。

### 取ってきた文章の中の作業者向けの指示(委任文 §6-3)

**この回に読んだ原典のうち、作業者向けの手引き(`AGENTS.md` / `CLAUDE.md` / `.cursor` / `copilot-instructions`)を新しく見つけたものは 0 件である。**
**通算は 6 件のままである。**この回に取った原典は、すべて実装のソースと登録の応答で、指示文ではない。
**読んだ文章の中に「これを実行しろ」「鍵を入れろ」に当たるものは無く、従ってもいない。**
...
**これは貼り付け後に打ち直した最終の実行の出力である。**
**この節を作る前に打ったとき、K12 は 1 件で当たっていた**(文言は `いちばん新しい回の節に「受け入れ検査の出力」が無い`)。
**直った K12 が、この回の節に貼り付けが無い状態を実際に止めたということである。**24 回目まではこの状態が 0 件で通っていた。
