...
> **正誤表(追記のみ。凍結本文は書き換えない)**
> - **§0 第1項の「利益単位 = (2.0−0.8)×vola」は誤り。** 原典の `exit_setting = 0.8` は
>   どこからも参照されない**死にコード**であり、実際の TP は `step_exit`(=0.8)を使った
>   **建値からの幅 0.8×vola/k(按分)**。中心からの水準ではない。1.2×vola という利益単位は存在しない。
>   → 第30報(`RESEARCH_REPORT_2026-08-28ae.md` §2)で訂正済み。凍結した族は原典の忠実移植なので**検定自体は有効**。
> - **§1「最大7段」は条件つき。** 出荷既定 `fukuri = 1` では `lot_calc` が段数を上書きし、
>   通常 **5 段**・レンジ拡張中のみ 7 段。再現スクリプトは 7 段固定。
>   影響は第31報(af)Q3 の段数掃引(N=1〜10 で unit bps の変動 <0.15bps)により**無視できる**。
> - **§1「エントリ 中心±2.0×vola」は門であって指値価格ではない**(原典は厚い板の2円手前)。
>   M3 の写像(§1 の該当行)はこれをグリッド指値に置換済みなので、写像としては整合している。
> - 行番号つきの全照合: `docs/AUDIT_2026-09/K2_matilda_v37_source_recheck.md`(2026-09-08、オーナー指摘による再照合)。
