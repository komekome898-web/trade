...
| tape(約定・ticker・板 top10 の日次抽出。`paper_logs/tape/`) | `paper_logs/tape/{executions,ticker,board_top10}_YYYYMMDD.csv.gz` | 2026-08-20〜(継続。**09-18 07:24〜09-21 02:36 UTC の穴は 2026-09-21 14:39 JST の共有 7aaa2ba で埋まった**: 09-18 全日 86,283 / 09-19 32,590 / 09-20 46,886 行の約定、ticker・板は各日 00:00〜23:59、最大間隔は日次メンテ 19:00〜19:12 UTC のみ) | 取得済 | 2026-09-21 | L-338〜L-340(`fetch_all.bat` が `logs\fetch.out.log` に追記できず 3 日止まっていた → ログ先を `logs\fetch_all.out.log` に変更。持ち主は不明のまま) | 研究の主材料(bitFlyer 側)。WS 再接続の ±2 分は約定が欠ける(上の API 取り直しの行) |
| 約定(公開 API `getexecutions` の取り直し。tape の穴埋め。列 `ts,price,size,side,id`、ミリ秒精度) | `backtest_data/bitflyer_executions_backfill_20260921/` | 2026-09-18 07:24 UTC〜09-21 01:39 UTC(151,079 件。日次メンテ 19:00〜19:12 UTC 以外に 4 分超の空白なし。tape と重なる 09-18 07:24:00〜44 の 92 件は (price,size,side) が完全一致) | 取得済 | 2026-09-21 | `scripts/fetch_bitflyer_executions_range.py`、`.progress.json`(303 頁)、`ACTION_LOG` 068 | tape 共有の穴(09-18 07:24 UTC〜、L-325、P15 ①)の橋渡し。**ticker・板 top10 はこの経路に無い**。**P15 ① 完了後の突き合わせ(L-340、共有 7aaa2ba)**: 重なる 09-18 07:24〜09-21 01:39 UTC で tape 147,173 行 / API 151,079 行。API だけ 4,173 行(2.8%)、tape だけ 267 行。API だけの 4,173 行の 47%(1,967)は WS 記録器の再起動の ±2 分に集中(ws ファイルの開始時刻と一致)= **tape は WS の再接続のたびに数秒〜数十秒の約定を落とす。約定だけなら API の取り直しの方が完全**。tape だけの 267 行は未解明(秒単位の突き合わせ。分単位では 3 行) |
...
| tardis 形式 約定(**標本日 = 各月 1 日**) | `data/tardis/bitflyer_FX_BTC_JPY_trades/` | **2019-09-01〜2026-09-01、85 標本日。全日 00:00〜23:59 のフルカバー、1 日 1.0 万〜290 万件。列 = exchange/symbol/timestamp/local_timestamp/id/side/price/amount(**約定の向きが付いている**)。`_fetch_summary.json` に取得時の遅延分布、`MD5SUMS` あり** | 取得済 | **中身を実際に開いて確認 2026-09-13**(それまで「詳細未確認」のまま 2 日放置していた) | — | **O-3c の経費実測(2023-06〜2024-10 に 17 標本日(**訂正 2026-09-18、`ls` で数え直し: 清算の窓 2023-06-25〜2024-10-14 に入るのは 2023-07-01〜2024-10-01 の 16 標本日。17 は 2023-06-01 を含めた月数**)が入る。板は無いが約定の向きがあるので実効スプレッドが測れる)** | **追記 2026-09-21: この実行環境は会話ごとに clone から作られ gitignore 域の `data/` は残らない(この会話の容器に無かった = 09-13 の「取得済」は別の容器)。同日、`fetch_tardis_samples.py --start 2023-07 --end 2024-10` で 16 標本日を取り直した(16/16 OK、9.7 MB、MD5SUMS あり)。利用規約 9.2(`https://docs.tardis.dev/legal/terms-of-service`)により生データは共有しない = 集計だけを `backtest_data/` に置く。**
...
| Coinalyze 清算の 1 分足集計(Bybit BTCUSDT / BTCUSD、OKX BTC-USDT-SWAP / BTC-USD-SWAP。列 `t,l,s` = 分の先頭 UTC 秒・ロング清算量・ショート清算量。線形は BTC 建て、インバースは USD 建て。**清算のあった分だけ返る**) | `backtest_data/coinalyze_liquidations_20260921/`(`raw/` に生応答 18 窓、`markets.json`、`.progress.json`) | 2026-09-14 21:00〜09-19 08:00 UTC(自前記録の穴 Bybit 09-16 00:47〜09-19 07:09 / OKX 09-14 21:57〜09-19 07:09 を含む。点数: BTCUSDT.6 892 / BTCUSDT_PERP.3 804 / BTCUSD.6 121 / BTCUSD_PERP.3 73) | 取得済(オーナー PC で実行、L-328) | 2026-09-21 | **自前 Bybit 記録との突き合わせ(重なる 09-14 21:00〜09-16 00:47 の 1,667 分)**: 清算のあった分 Coinalyze 247 / 自前 246、両方 246、合計量 291.703 BTC = 291.703 BTC、分ごとの相関 1.0000。Coinalyze `l` = Bybit `S=Buy`(226.152)、`s` = `S=Sell`(65.551)。Coinalyze だけの 1 分(09-14 21:59、0.01)は自前の再接続の間(記録器ログ 21:59:11〜22:00:15)。**OKX は突き合わせ未実施**。1 件ごとの記録の代わりにはならない(分の中の並びと間隔は無い) | P15 ②(L-325)。`scripts/fetch_coinalyze_liquidations.py` |
...
| Binance/BitMEX/Bybit/OKX 清算 WS ストリーム(自前記録) | `data/liquidations/*.jsonl.gz`, `paper_logs/liquidations/*.jsonl.gz` | 常駐記録(取引所により履歴無し、詳細 `scripts/record_liquidations.py`) | 取得済。うち **11 ファイルは破損 → 2026-09-17 に回収済み**(`backtest_data/liquidations_repaired_20260917/`、59,944 行、`ACTION_LOG` 050。2026-09-17 実測 `ls paper_logs/liquidations/*.trunc1.jsonl.gz | wc -l` = 11。09-11 時点の記載は 10。旧 Writer のハードキルによる gzip メンバ境界破損、2026-09-11 発見、L-121。回収手順 `docs/OWNER_PROCEDURES.md` P14) | 2026-09-11 | — | 未使用 |
...
| Binance COIN-M BTCUSD_PERP metrics(建玉`sum_open_interest`・L/S比、5分粒度) | `backtest_data/binance_cm_o3c_20260913/metrics/BTCUSD_PERP/`(日次zip、379/478日) | 2023-06-25〜2024-10-14、欠測99日(`2023-09-25`・`2023-11-19` の単発 2 日 + `2024-03-04`〜`2024-06-08` の 97 暦日。**この 97 暦日のうち `2024-06-01` は 清算の zip も無い日で、判定区間に入っていない**)。**段 A の判定区間内の欠けも 97 日だが、別の集合である**: `2023-11-19` + その 96 日(= `2024-03-04`〜`2024-05-31` の 89 日 + `2024-06-02`〜`2024-06-08` の 7 日)= 97 日(一覧は `docs/PHASE2/O3C/PRICE_LEVEL/REACTION_PREREG_2026-09-18.md` §8.6) | 取得済(配信停止前の退避)。**欠測 99 日は公開アーカイブ側にも無い(2026-09-18 実測: daily の zip と .CHECKSUM 198 URL すべて HTTP 404、S3 のキー一覧 1,791 zip にも不在)。COIN-M に monthly/metrics のツリーが無く monthly 経由の代替も無い。REST dapi はこの環境から HTTP 451。第三者(Coinalyze / CoinGlass / tardis)は鍵が要る(未取得)。オーナー PC は未確認** | 2026-09-18 | `docs/DATA/probes/20260913_binance_cm_o3c_fetch.log`、`docs/DATA/probes/20260918_cm_metrics_missing_days.log` | O-3c 段 A(`REACTION_PREREG_2026-09-18.md` §8.6 / §10.3) |
| Binance COIN-M metrics の欠測の広がり(BTCUSD_PERP / ETHUSD_PERP / USD-M BTCUSDT の比較)と配信の継続 | `https://data.binance.vision/data/futures/{cm,um}/daily/metrics/`(S3 キーの全件列挙) | 窓 2023-06-25〜2024-10-14 で BTCUSD_PERP 欠測 99 日 / ETHUSD_PERP 欠測 98 日(99 日から `2023-11-19` を除いた集合)/ USD-M BTCUSDT 欠測 0 日。**COIN-M daily metrics のキーは 2021-07-08〜2026-09-17(zip 1,791 件)で 2024-10-14 で止まっていない。止まっているのは liquidationSnapshot(472 件、2023-06-25〜2024-10-14)** | 未取得(到達性とキー一覧のみ実測。本体は取得していない) | 2026-09-18 | `docs/DATA/probes/20260918_cm_metrics_missing_days.log` | O-3c 段 A(欠測の射程・2 周目の材料) |
...
| CoinGlass 建玉履歴(`/api/futures/open-interest/history`) | `https://open-api-v4.coinglass.com/api/futures/open-interest/history` | 該当なし | 試行して不可(HTTP 200、本文 `{"code":"401","msg":"API key missing."}`。方法: GET、鍵無し。実測 2026-09-17) | 2026-09-17 | `docs/DATA/probes/20260917_o3c_missing.log` | O-3c 段 2 の 3 件目(`MISSING_2026-09-17.md` §1) |
| Binance COIN-M metrics のレバレッジ関連列(`count_toptrader_long_short_ratio` / `sum_toptrader_long_short_ratio` / `count_long_short_ratio`) | `backtest_data/binance_cm_o3c_20260913/metrics/BTCUSD_PERP/` | **在庫の 379 日・109,098 行すべてで 3 列とも空(実測 2026-09-18)。**`sum_taker_long_short_vol_ratio` は空 167/109,098、`sum_open_interest` は空 0。前回記載のサンプル 7 日・2,016 行はこの部分集合 | 取得済(列自体は空)。**取り直しても同じで md5 が在庫と一致 = 出所側が空(実測 2026-09-18)。**経路 3 つを当てた: ① USD-M アーカイブ BTCUSDT は 8 列とも埋まる(別商品)/ ② COIN-M アーカイブ再取得は空 / ③ COIN-M REST dapi はこの環境から HTTP 451。**オーナー PC は未確認** | 2026-09-18 | `docs/DATA/probes/20260918_o3c_cm_metrics_ls_ratio.log`(2026-09-17 の初出は `docs/AUDITOR/ACTION_LOG.md` 049) | O-3c 段 2 の 3 件目(§9)/ 段 A(`REACTION_RUN12_2026-09-18.md` §4.8) |
| Binance COIN-M aggTrades の読み込み可否(`PriceSeries.from_trades` 用) | `backtest_data/binance_cm_o3c_20260913/aggTrades/BTCUSD_PERP/` | zip 475 日(`.CHECKSUM` 別に 475)。列 `agg_trade_id,price,quantity,first_trade_id,last_trade_id,transact_time,is_buyer_maker`、`transact_time` は ms | 取得済(`(transact_time, price)` をそのまま渡せる形。実行は未) | 2026-09-17 | `docs/AUDITOR/ACTION_LOG.md` 049 | O-3c 段 2 の 3 件目(§5) |
