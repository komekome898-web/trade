#!/usr/bin/env python3
"""Item 3, round 1, table-maker: build the six blind comparison tables from
...
import i3_targets as TG  # noqa: E402
...
def load_viewpoints() -> None:
    """Viewpoint id -> title, from the '### C3-n title(...)' headings of DEFINITIONS.md."""
    import re
    for line in (BATTERY / "DEFINITIONS.md").read_text(encoding="utf-8").splitlines():
        m = re.match(r"### (C3-\d+) (.+?)(（|\(|$)", line)
        if m:
