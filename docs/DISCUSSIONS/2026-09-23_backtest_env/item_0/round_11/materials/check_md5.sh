...
for p in current survey mutant; do
  ...
  md5sum $B/round_11/$a $B/round_11/$b
  if cmp -s $B/round_11/$a $B/round_11/$b; then echo "$p: identical ($a $b)"; else echo "$p: differ ($a $b) :: $(cmp $B/round_11/$a $B/round_11/$b 2>&1)"; fi
done
for d in pass2_tables pass2_tables_ownlog; do
  ...
  for t in current_1 current_2 survey_1 survey_2 mutant_1 mutant_2; do
    a=$B/round_11/$(f $M $t); b=$M/$d/$(f $M/$d $t)
