#!/bin/sh
# Item 0, round 6, materials person: run every target through the scene set
...
# reproduction opponents/repro_*.py (round 6: repro_lean52).
# Usage (from the repo root): sh docs/DISCUSSIONS/2026-09-23_backtest_env/item_0/round_6/materials/run_all.sh
set -u
REPO=$(pwd)
M=docs/DISCUSSIONS/2026-09-23_backtest_env/item_0/round_6/materials
...
run() {  # $1 target, $2 interpreter, $3 venv label
  ...
  # round 6: rc is the runner's own exit status (round 5 took the rc of `tail`)
