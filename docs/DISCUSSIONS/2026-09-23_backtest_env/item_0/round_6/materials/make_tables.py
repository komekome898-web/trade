#!/usr/bin/env python3
"""Build the six blinded comparison tables of item 0, round 6 (materials
...
Output:
  * six files `round_6/表_<6 random chars>.md` (rows "A" / "B", columns =
...
def _error_kind(detail: str) -> str:
    """What came out, in words that name no tool (round 6): a provenance
...
def failure_notes(runs: dict[str, dict[str, dict]], merged: dict[str, dict]) -> list[str]:
    ...
    with what came out and the measured seconds of its whole run (round 6:
