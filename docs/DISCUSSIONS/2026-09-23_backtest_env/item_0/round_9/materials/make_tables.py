#!/usr/bin/env python3
"""Build the six blinded comparison tables of item 0, round 9 (materials
...
Output:
  * six files `round_9/表_<6 random chars>.md` (rows "A" / "B", columns =
...
def _error_kind(detail: str) -> str:
    """What came out, in words that name no tool (round 9): a provenance
...
def _durations() -> dict[str, int]:
    ...
    out = {}
    # round 9: the survey targets were reused from round 8 (same scene-set
    # fingerprint, L-435); their measured lengths are round 8's log lines,
    # copied by run_all.sh into logs/run_all_round8_reused.log
    lines = []
    for log in (MAT / "logs" / "run_all.log", MAT / "logs" / "run_all_round8_reused.log"):
        if log.exists():
...
def failure_notes(runs: dict[str, dict[str, dict]], merged: dict[str, dict]) -> list[str]:
    ...
    with what came out and the measured seconds of its whole run (round 9:
...
def candidate_notes(path, merged_targets: list[str]) -> list[str]:
    ...
    if not not_ran:
        return []
    # round 9: configured targets `base@label`; the candidate number is the
...
def main() -> None:
    ...
    # round 9: the one sentence the scene keeper's DEFINITIONS.md requires in
    # every table a judge reads (L-438 (2), handoff to the materials person)
    type_note = ["型の組は対象の持つ型から規則で決めた"]
    survey_notes = type_note + survey_notes
    pairs = {
        "current_1": (new, cur, type_note, "new_impl", "current_impl"),
        "current_2": (cur, new, type_note, "current_impl", "new_impl"),
        "survey_1": (new, survey, survey_notes, "new_impl", "survey_best"),
        "survey_2": (survey, new, survey_notes, "survey_best", "new_impl"),
        "mutant_1": (new, mut, type_note, "new_impl", "mutant"),
        "mutant_2": (mut, new, type_note, "mutant", "new_impl"),
    }
    ...
    # round 9 (DEFINITIONS.md handoff): per configured target, the chosen
