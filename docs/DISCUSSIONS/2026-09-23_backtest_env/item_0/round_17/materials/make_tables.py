#!/usr/bin/env python3
"""Build the six blinded comparison tables of item 0, round 17 (materials
...
Output:
  * six files `round_17/表_<6 random chars>.md` (rows "A" / "B", columns =
...
def _error_kind(detail: str) -> str:
    """What came out, in words that name no tool (round 17): a provenance
...
def _durations() -> dict[str, int]:
    ...
    out = {}
    # round 17: the scene-set fingerprint differs from round 16's, so every
...
def failure_notes(runs: dict[str, dict[str, dict]], merged: dict[str, dict]) -> list[str]:
    ...
    with what came out and the measured seconds of its whole run (round 17:
...
def candidate_notes(path, merged_targets: list[str]) -> list[str]:
    ...
    if not not_ran:
        return []
    # round 17: configured targets `base@label`; the candidate number is the
...
def main() -> None:
    ...
    # round 17: build the tables of pass 2 (runs_2/) elsewhere, only to check
    ...
    # round 17: the one sentence the scene keeper's DEFINITIONS.md requires in
    # every table a judge reads (L-438 (2), handoff to the materials person)
    type_note = ["型の組は対象の持つ型から規則で決めた"]
    # round 17: per row, the not-entered cells note of every configured
    ...
    # round 17 (DEFINITIONS.md handoff): per configured target, the chosen
    ...
    # round 17: every configured target's not-entered note line (names kept
