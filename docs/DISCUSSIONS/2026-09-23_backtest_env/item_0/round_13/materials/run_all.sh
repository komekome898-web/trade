#!/bin/sh
# Item 0, round 13, materials person: run every configured target of
...
# Round 13: the scene-set fingerprint differs from round 11's (round 12 has no
# materials), so every configured target is run (L-435). Candidate 35
...
# Usage (from the repo root): sh docs/DISCUSSIONS/2026-09-23_backtest_env/item_0/round_13/materials/run_all.sh
set -u
REPO=$(pwd)
