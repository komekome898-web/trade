#!/usr/bin/env python3
"""Build the six blinded comparison tables of item 0, round 8 (materials
...
Output:
  * six files `round_8/表_<6 random chars>.md` (rows "A" / "B", columns =
...
def _error_kind(detail: str) -> str:
    """What came out, in words that name no tool (round 8): a provenance
...
def failure_notes(runs: dict[str, dict[str, dict]], merged: dict[str, dict]) -> list[str]:
    ...
    with what came out and the measured seconds of its whole run (round 8:
...
def candidate_notes(path, merged_targets: list[str]) -> list[str]:
    ...
    if not not_ran:
        return []
    # round 8: configured targets `base@label`; the candidate number is the
...
def main() -> None:
    ...
    # round 8: the one sentence the scene keeper's DEFINITIONS.md requires in
    ...
    # round 8 (DEFINITIONS.md handoff): per configured target, the chosen
