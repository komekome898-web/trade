...
委任文: `docs/DATA/delegations/20260923_backtest_env_prompt.md`(指紋 `20260923_backtest_env_prompt.md@ea9bc00a5583`)。この資料は §3「要件と判定の固定」に従い、項目 0 の 1 周目の前に要件・観点・調査結果の側を固定する。**周回の途中でここを緩めない、後から厳しくもしない。**
...
- 「要件と判定の固定」の段落は 1 箇所のみ変更(「§0 の L-407 の行」→「§0 の L-407 と L-418 の行」。圧倒の判定・周回上限・未達停止条件を変えてよいのはオーナーだけ、という規定にどの逐語行が対応するかの参照が増えただけで、本ファイルが固定した比較の観点(P0-1〜P0-7)・調査結果の側・当方の現状のいずれの中身にも該当しない。
