#!/usr/bin/env python3
"""Build the six blinded comparison tables of item 0, round 5 (materials
...
Output:
  * six files `round_5/表_<6 random chars>.md` (rows "A" / "B", columns =
...
def failure_notes(runs: dict[str, dict[str, dict]], merged: dict[str, dict]) -> list[str]:
    ...
    for sc in SCENES:
        if sc.id in per_scene:
            kinds = "、".join(sorted(set(k for _, k in per_scene[sc.id])))
            took = "、".join(f"{secs[n]} 秒" if n in secs else "未記録" for n, _ in per_scene[sc.id])
            notes.append(
                f"場面 {sc.id}: 動かせた道具のうち {len(per_scene[sc.id])} 件は結果なし"
                f"(試したこと: 場面の定義どおりに対象の公開の口で走らせた。出たもの: {kinds}。"
                f"実測の時間: その道具を全場面に 2 回ずつ通した所要 {took}(打ち切りではなく例外で止まった)。"
                f"この場面の行は残りの道具の最も良い結果 = {merged[sc.id]['correctness']})。")
    return notes


def candidate_notes(path) -> list[str]:
    ...
    No names, no times (those stay in that file and in materials)."""
    if path is None:
        return []
    ...
    if not not_ran:
        return []
    kinds = collections.Counter(r["reason_kind"] for r in not_ran)
    parts = "、".join(f"{k} {n} 件" for k, n in sorted(kinds.items()))
    return [f"調査結果の側の候補 {len(rows)} 件のうち、動かせた {len(ran)} 件の結果だけをこの行に寄せた。"
            f"動かせなかった {len(not_ran)} 件(理由の内訳: {parts})はこの行に入っていない"
            f"(候補ごとの欠け。場面集の検討表で扱う)。"]


def render(a: dict[str, dict], b: dict[str, dict], notes: list[str]) -> str:
    lines = ["# 比較の表(項目 0、第 5 周)", "",
...
def main() -> None:
    ...
    survey_notes = list(a.survey_note) + candidate_notes(a.runnability) + failure_notes(survey_runs, survey)
