#!/bin/sh
# Item 0, round 5, materials person: run every target through the scene set
...
# the scene keeper (opponents/RUNNABILITY.tsv, result 走った = 36 tools).
# Usage (from the repo root): sh docs/DISCUSSIONS/2026-09-23_backtest_env/item_0/round_5/materials/run_all.sh
set -u
REPO=$(pwd)
M=docs/DISCUSSIONS/2026-09-23_backtest_env/item_0/round_5/materials
...
run() {  # $1 target, $2 interpreter, $3 venv label
  ...
  out=$(cd "$REPO" && BT_SCRATCH=$S PYTHONPATH=src timeout 400 "$2" -B $RUNNER --target "$1" --out $M/runs/"$1".tsv 2>$M/logs/"$1".stderr | tail -1)
  rc=$?
