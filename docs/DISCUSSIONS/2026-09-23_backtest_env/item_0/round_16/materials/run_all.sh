#!/bin/sh
# Item 0, round 16, materials person: run every configured target of
...
# Round 16: the scene-set fingerprint differs from round 15's (cb42e201...
# -> 38460620...), so every configured target is run (L-435). Candidate 35
# (opp_thirupathikannan_execsim, added in round r13-1) runs under the venv the
# scene keeper made inside its copy (c35_run/.venv). Two whole passes
...
# Usage (from the repo root): sh docs/DISCUSSIONS/2026-09-23_backtest_env/item_0/round_16/materials/run_all.sh
set -u
REPO=$(pwd)
M=docs/DISCUSSIONS/2026-09-23_backtest_env/item_0/round_16/materials
...
venv_of() {  # base target -> venv directory name ("" = repo python3)
  case "$1" in
    ...
    opp_thirupathikannan_execsim) echo c35 ;;
    *) echo "UNKNOWN" ;;
  esac
}
...
for t in $(cat $LD/list_targets.out); do
  ...
  if [ "$v" = "" ]; then
    ...
    run "$t" "$V/c35_run/.venv/bin/python" c35_run/.venv
  elif [ "$v" = "UNKNOWN" ]; then
