#!/usr/bin/env python3
"""Build the six blinded comparison tables of item 0, round 16 (materials
...
Output:
  * six files `round_16/表_<6 random chars>.md` (rows "A" / "B", columns =
...
def _error_kind(detail: str) -> str:
    """What came out, in words that name no tool (round 16): a provenance
...
def failure_notes(runs: dict[str, dict[str, dict]], merged: dict[str, dict]) -> list[str]:
    ...
    with what came out and the measured seconds of its whole run (round 16:
...
def candidate_notes(path, merged_targets: list[str]) -> list[str]:
    ...
    if not not_ran:
        return []
    # round 16: configured targets `base@label`; the candidate number is the
...
def main() -> None:
    ...
    # round 16: build the tables of pass 2 (runs_2/) elsewhere, only to check
    ...
    # round 16: the one sentence the scene keeper's DEFINITIONS.md requires in
    # every table a judge reads (L-438 (2), handoff to the materials person)
    type_note = ["型の組は対象の持つ型から規則で決めた"]
    # round 16: per row, the not-entered cells note of every configured
    ...
    # round 16 (DEFINITIONS.md handoff): per configured target, the chosen
    ...
    # round 16: every configured target's not-entered note line (names kept
