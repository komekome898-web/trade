#!/usr/bin/env python3
"""Build the six blinded comparison tables of item 0, round 14 (materials
...
Output:
  * six files `round_14/表_<6 random chars>.md` (rows "A" / "B", columns =
...
def _error_kind(detail: str) -> str:
    """What came out, in words that name no tool (round 14): a provenance
...
def _durations() -> dict[str, int]:
    ...
    out = {}
    # round 14: the scene-set fingerprint is the same as round 13's, so the
    # results of current_impl and of the survey-side targets are round 13's
    # (L-435, copied into runs/ and runs_2/); their measured lengths are
    # round 13's pass-1 log, copied as logs/run_all_reused_round13.log. The
    # targets run this round (new_impl, mutant) are in logs/run_all.log.
    lines = []
    for log in (LOGS / "run_all_reused_round13.log", LOGS / "run_all.log"):
        if log.exists():
...
def failure_notes(runs: dict[str, dict[str, dict]], merged: dict[str, dict]) -> list[str]:
    ...
    with what came out and the measured seconds of its whole run (round 14:
...
def candidate_notes(path, merged_targets: list[str]) -> list[str]:
    ...
    if not not_ran:
        return []
    # round 14: configured targets `base@label`; the candidate number is the
    # trailing digits of the base name
    rewritten = {m.group(1) for t in merged_targets if t.startswith("repro_")
                 for m in [re.search(r"(\d+)$", t.split("@", 1)[0])] if m}
    ...
    return [f"調査結果の側の候補 {len(rows)} 件のうち、この行に寄せたのは動かせた {len(ran)} 件と、"
            ...
            f"(計 {len({t.split('@', 1)[0] for t in merged_targets})} 件。設定の違いを別に数えると {len(merged_targets)} 対象)。残りの {len(rest)} 件(理由の内訳: {parts})はこの行に入っていない"
            f"(候補ごとの欠け。場面集の検討表で扱う)。"]


def not_entered_notes(label: str, targets: list[str]) -> list[str]:
    """Round 13 (DEFINITIONS.md handoff, round r13-1): the scene keeper's
    note line `grid_c.not_entered_note` for every configured target merged
    into row `label`. One target -> its line; many (the survey row) -> the
    identical lines grouped, each with how many configured targets gave it
    (no target name in the table; the per-target lines are in
    materials/not_entered.tsv)."""
    import grid_c
    lines = collections.Counter(grid_c.not_entered_note(RUNS / f"{t}.tsv", SCENES) for t in targets)
    if len(targets) == 1:
        return [f"行 {label} の対象: {next(iter(lines))}"]
    return [f"行 {label} に寄せた設定つき対象 {len(targets)} 件のうち {n} 件: {line}"
            for line, n in sorted(lines.items(), key=lambda kv: (-kv[1], kv[0]))]


def render(a: dict[str, dict], b: dict[str, dict], notes: list[str]) -> str:
    lines = ["# 比較の表(項目 0、第 14 周)", "",
...
def main() -> None:
    ...
    # round 14: build the tables of pass 2 (runs_2/) elsewhere, only to check
    ...
    # round 14: the one sentence the scene keeper's DEFINITIONS.md requires in
    # every table a judge reads (L-438 (2), handoff to the materials person)
    type_note = ["型の組は対象の持つ型から規則で決めた"]
    # round 14: per row, the not-entered cells note of every configured
    # target merged into that row (grid_c.not_entered_note), after the type
    # sentence and before the survey-side notes
    members = {"new_impl": ["new_impl"], "current_impl": ["current_impl"], "mutant": ["mutant"],
               "survey_best": survey_targets}

    def notes_for(la: str, lb: str, rest: list[str]) -> list[str]:
        return type_note + not_entered_notes("A", members[la]) + not_entered_notes("B", members[lb]) + rest

    pairs = {
        "current_1": (new, cur, notes_for("new_impl", "current_impl", []), "new_impl", "current_impl"),
        "current_2": (cur, new, notes_for("current_impl", "new_impl", []), "current_impl", "new_impl"),
        "survey_1": (new, survey, notes_for("new_impl", "survey_best", survey_notes), "new_impl", "survey_best"),
        "survey_2": (survey, new, notes_for("survey_best", "new_impl", survey_notes), "survey_best", "new_impl"),
        "mutant_1": (new, mut, notes_for("new_impl", "mutant", []), "new_impl", "mutant"),
        "mutant_2": (mut, new, notes_for("mutant", "new_impl", []), "mutant", "new_impl"),
    }
    ...
    # round 14 (DEFINITIONS.md handoff): per configured target, the chosen
    # values (`choose`) and, for the scenes whose type combination comes from
    # the target, the chosen combination (`types_1`), copied from the runs
    with (MAT / "configs.tsv").open("w", encoding="utf-8") as f:
        f.write("target\tchoose\tscene_id\ttypes_1\n")
        for t in targets:
            rows = load(t)
            chooses = sorted({rows[sc.id].get("choose", "") for sc in SCENES})
            for sc in SCENES:
                ty = rows[sc.id].get("types_1", "")
                if ty not in ("", "null"):
                    f.write(f"{t}\t{'|'.join(chooses)}\t{sc.id}\t{ty}\n")
            f.write(f"{t}\t{'|'.join(chooses)}\t(all)\t-\n")
    # round 14: every configured target's not-entered note line (names kept
    # here in materials, never in a table)
    import grid_c
    with (MAT / "not_entered.tsv").open("w", encoding="utf-8") as f:
        f.write("target\tnote\n")
        for t in targets:
            f.write(f"{t}\t{grid_c.not_entered_note(RUNS / f'{t}.tsv', SCENES)}\n")
