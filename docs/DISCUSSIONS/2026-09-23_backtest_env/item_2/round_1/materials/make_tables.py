#!/usr/bin/env python3
"""Item 2, round 1, table-maker: build the six blind comparison tables from
...
(VIEWPOINTS below, from the scenes' own viewpoint field and DEFINITIONS.md headings).  Every cell has two fields: correctness (class_1 of
...
BATTERY = REPO / "tests/bt/battery/item_2"
sys.path.insert(0, str(BATTERY))

import i2_scenes as S  # noqa: E402
import i2_targets as TG  # noqa: E402
...
def notes_for(label: str, name: str, row: dict, n_survey: int) -> list[str]:
    out = []
    if name == "survey_best":
        n_venv = sum(1 for k in TG.SURVEY if not k.startswith("opp_repro_"))
        n_rep = sum(1 for k in TG.SURVEY if k.startswith("opp_repro_"))
        out.append(f"注記: 行 {label} は {n_survey} 対象(道具の設定 {n_venv} 件(1 つの道具の 2 つの設定は別々に数える)と、一次資料どおりに最小限で書き直した機構 {n_rep} 件)の"
    ...
    for s in S.SCENES:
        ...
        if name == "survey_best":
            ...
            out.append(f"注記: 場面 {s['id']}: 行 {label} に寄せた {n_survey} 対象の全部が結果なし。試したこと: 場面の定義どおりに、"
                       ...
                       f"実測の時間: 1 回の実行でこの場面にかかった時間の最長 {secs:.3f} 秒(時間の上限は 1 場面 120 秒)。")
        else:
            secs = c["secs"]
...
def load_viewpoints() -> None:
    """Viewpoint id -> title, from the '### C2-n title(...)' headings of DEFINITIONS.md."""
    import re
    for line in (BATTERY / "DEFINITIONS.md").read_text(encoding="utf-8").splitlines():
        m = re.match(r"### (C2-\d+) (.+?)(（|\(|$)", line)
        if m:
            VIEWPOINTS[m.group(1)] = m.group(2).strip()
