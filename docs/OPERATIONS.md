...
### 商品仕様の確認(2026-08-20 実測、旧 NOTE_CRYPTO_CFD_2026-08-20.md より)
...
- モジュールの**価値**の判定は champion のペーパー取引の部分集合で行う
  (常設基準は 2026-09-08 の全捨てで失効。新しい基準を事前登録してから判定する)。
  「モジュール入りのバックテストが勝った」は採用根拠になりません。
...
バーストスキャルパーは 2026-08-21 に停止し**退役**しました(停止根拠だった判定は
2026-09-08 の全捨てで失効。スクリプトは `scripts\run_scalp_paper.py` に保存されており、
復帰は start_all.bat に1行戻すだけ)。
...
JPX(先物 ON1・現物 ETF 板寄せ)の実弾ジョブ手順は `docs/OPERATIONS_JPX.md` に分離した。
本書(暗号資産 CFD 側)からは読者が異なるため §5.1・§5.2 は移設済み(内容の変更なし)。
...
4. `UserPromptSubmit`(`owner_turn_digest.sh`、2026-09-12、L-130)— オーナーの発言のたびに
   ...
   太字で始まる行(進行中の項目と恒久規則)、**オーナーの逐語による規則 2 つ**(日本語のみ L-126 /
   有料インフラを提案しない L-124)と、**リードが導いた規則(オーナーの指示ではない)**
   (PC の状態は届いたものから自分で確かめる。経緯 L-122)。**出所を分けて表示するのは 2026-09-13
   の L-162 による** — それまでは 3 つとも「規則」と並べて表示しており、3 つ目はオーナーが
   言っていないものだった。**状態板に書くだけでは読まれない**(I-006 の根本原因)ので、読む側を
