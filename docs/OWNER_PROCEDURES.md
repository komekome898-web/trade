...
2. `deploy\probe_latency.bat` をダブルクリック(または PowerShell で実行)。
   **(2026-09-12 注記)** 09-12 09:22 の共有時点で `data\latency\api_probe.csv` も `logs\latency_probe.out.log` も
   届いていない = プローブは起動していないか、起動直後に落ちている(原因は次の共有の `latency_probe.out.tail.log` で見る)。
   `start_all.bat` の常駐リストに入れて夜間再起動で自動起動させる案は、PC で常時動くものの変更なので
   **オーナー承認待ち**(監査役の指摘。承認されるまでこの手順のまま)。
