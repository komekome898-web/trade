...
2. **(2026-09-12 改訂、L-132 の後)** プローブは `start_all.bat` の常駐リストに入れたので、次の
   `restart_all.bat`(夜間 04:02 の自動実行を含む)から**自動で起動する**。手動の `deploy\probe_latency.bat` は
   不要(押しても二重起動しない)。以下は改訂前の手順で、参考として残す。
   `deploy\probe_latency.bat` をダブルクリック(または PowerShell で実行)。
