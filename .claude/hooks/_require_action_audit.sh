...
# **鍵は行頭に固定する(2026-09-13、13 本目の監査)。**
# 旧版は `index($0,k)` で**行のどこにあっても一致**した。ところがこの台帳自身が、
# 監査役の指摘を逐語で載せるために**地の文の中に鍵の文字列をそのまま引用している**
# (例:「つまり `抜き取り: <日付>` の 1 行だけを足せば…」)。引用が本物のマーカーより
# 前に来れば、そこから節が始まってしまう。**兄弟の awk(下の LAST_SAMPLE_VERDICT)は
# 行頭固定なのに、直した当の側だけ固定されていなかった** — 10〜12 本目が繰り返し指摘した
# 「同じ種類の穴を一方だけ塞ぐ」型の再発。
