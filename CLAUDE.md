...
- テスト: `PYTHONPATH=src python -m pytest -q`(現在976件)
...
- 研究の判定と問いの設定は規則で自動。オーナーが決めるのは取り返しのつかないこと(実弾・口座・資本・新市場・リスク上限)だけ。
...
- 単位の着手前に調達票とリード盲点監査。週次要約は毎週月曜 日本時間 08:00(`docs/WEEKLY_DIGEST/`)。
