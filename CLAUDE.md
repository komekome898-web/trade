...
- **オーナーに見える文はすべて日本語で書く(オーナー恒久規則 2026-09-12、L-126)。** フックの催促・システム通知・バックグラウンド作業の完了への応答も例外ではない(それらもオーナーに表示される)。英語はコード・コミットメッセージ・bat のコメントだけ。
...
- 監査役の定義(`.claude/agents/owner-auditor.md`)と既知解・原則の**変更はオーナー承認制**(L-113。オーナー逐語「**要約された内容では『何の事を指しているか』『この定義は効くのか』という判断ができないので承認できません**」)。リードは提案だけ。
...
- 判定の**実行**は規則で自動。ただし**枠組み(問いの立て方・族の切り方・何を検定単位にするか)はオーナーに見せてから進める**。**これはリードが導いた規則であって、オーナーの指示ではない**(2026-09-13、L-162 で判明。「枠組み」という語はオーナーの逐語に一度も現れず、OWNER_LOG で 18 回ともリードの対応列にあった)。**経緯となったオーナーの言葉**: 「**盲目的に全捨てが妥当。KNOWLEDGE.md すらも捨てれば同じ過ちを繰り返さない**」(L-019、2026-09-08)— 計算ではなく枠組みの誤りが最も高くついたとリードが判断し、この規則を作った。オーナーが決めるのは取り返しのつかないこと(実弾・口座・資本・新市場・リスク上限)と、この枠組み。**サーバー・VPS・有料インフラの賃借は収益の算段が立つまで提案しない(オーナー恒久規則、L-124。過去に複数回言われていたが文書化されていなかった)。**
