...
- 納品物(事前登録・報告・調査・オーナーへの手順の指示)は**監査役を通さずにオーナーへ出さない**。委任先の調査報告もリードが使う前に監査役を通す。
- 監査役の問いには 1 件ずつ書面で答える(`docs/AUDITOR/VERDICTS/`)。**「止める」はリードが単独で退けられない**: 直すか、両論を添えてオーナーへ上申する。「誤警報」と書くには 1 件ずつ読んで理由を書く(L-110)。
- 監査役の定義(`.claude/agents/owner-auditor.md`)と既知解・原則の**変更はオーナー承認制**。リードは提案だけ。
...
- 判定の**実行**は規則で自動。ただし**枠組み(問いの立て方・族の切り方・何を検定単位にするか)はオーナーに見せてから進める** — 2026-09-08 の全捨てで、計算ではなく枠組みの誤りが最も高くついたと判明したため。オーナーが決めるのは取り返しのつかないこと(実弾・口座・資本・新市場・リスク上限)と、この枠組み。**サーバー・VPS・有料インフラの賃借は収益の算段が立つまで提案しない(オーナー恒久規則、L-124。過去に複数回言われていたが文書化されていなかった)。**
