"""Row-fraction splits: the compatibility mouth of the old
`bot.backtest.walk_forward` (item 4, old item 14: `split_data`) and the
native split (`split_rows`, rules D-1 / D-2 of the item-4 battery).
  ...
  split_rows(n, train_frac, val_frac, arithmetic)
      ...
      refused. arithmetic "decimal": the fractions are the written decimal
      values (0.7 + 0.2 = 0.9 exactly); "binary": the old module's float
      arithmetic (0.7 + 0.2 = 0.8999999999999999, so 100 rows split at 89).
  split_data(candles, train_frac=0.6, val_frac=0.2)
      the old name and arguments, the old (binary) arithmetic.
"""
...
def split_bounds(n: int, train_frac: float, val_frac: float, arithmetic: str) -> tuple[int, int]:
    ...
    if arithmetic == "binary":
        if not 0 < t < 1 or not 0 < v < 1 or t + v >= 1:
            raise SplitError("fractions must be in (0,1) and sum to < 1")
        return int(n * t), int(n * (t + v))
    if arithmetic == "decimal":
    ...
    raise SplitError(f"arithmetic must be binary or decimal, got {arithmetic!r}")
...
def split_data(candles: pd.DataFrame, train_frac: float = 0.6,
    ...
    if not 0 < train_frac < 1 or not 0 < val_frac < 1 or train_frac + val_frac >= 1:
        raise SplitError("fractions must be in (0,1) and sum to < 1")
    t_end, v_end = split_bounds(len(candles), train_frac, val_frac, "binary")
