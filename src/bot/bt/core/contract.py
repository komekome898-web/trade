...
CORE_CONTRACT: dict = {
    ...
    "visibility": {
        ...
        "history_limit": "per event type the latest N..2N delivered events are kept; the overall history "
                         ...
                         "dropped event for the whole run, plus one chunk per drop (an array('q') of that "
                         "drop's facts -- a drop removes history_limit + 1 events of one type: by "
                         "sys.getsizeof 80 bytes plus 16 per dropped event, and 8 for its slot in the pending "
                         "list, on CPython 3.11) until the strategy's next read that needs them folds the "
                         "chunks in, "
                         ...
                         "88.2 bytes per fact (round 12, measured again in round 13)); a read raises "
                         "HistoryTruncatedError exactly when its "
        ...
        "process_state": "what the core accepts, refuses or computes is decided by the value and by objects "
                         ...
                         "(round 15, i0-r14-05): an entry that takes or hands out plain data -- values.freeze, "
                         "settle, renew, thaw, and place_order, order() and extra_dict() called from inside "
                         "on_event (the ones measured) -- needs at most " + str(CALL_FRAMES) + " frames of the "
                         "interpreter's recursion limit below its caller whatever the nesting (measured: at "
                         "most 24 since round 17 (22 before), place_order from inside on_event with a FrozenDict "
                         "key), and with that much headroom whether a "
                         ...
                         "comparison (values._plain_equal: no frame per level, round 16, i0-r15-03, and none "
                         "per candidate when the other side holds several keys of one hash, round 17, "
                         "i0-r16-01: the candidates are tried as tasks on the same explicit stack) -- measured "
                         "(freeze of a dict with the two as keys): tuples and frozensets need the levels + 6 "
                         "frames of headroom (97 levels: 103), FrozenDicts at most 12 at any depth, a mixed "
                         "nest at most 14, chains whose keys collide at every level (3 to 5 keys of one hash a "
                         "level, 1 to 90 levels) at most CALL_FRAMES + levels; at most " +
                         str(CALL_FRAMES) + " + one frame per level for every container kind "
                         "(tests/bt/item_0/test_bt0_r16_one_reading.py grid F, test_bt0_r17_shared_and_placed.py "
                         "grid C); with less, "
                         ...
                         "16, i0-r15-02). The work of taking, handing out and comparing plain data is decided "
                         "by the OBJECTS handed over (round 17, i0-r16-03): every walk (freeze, settle, renew, "
                         "thaw) rebuilds a container once per object -- one the value holds twice comes out as "
                         "one new container held twice, as given -- a FrozenDict's hash is made once, when it "
                         "is first asked for (the core asks only for a dict key or a set element, which the "
                         "sender's own dict or set hashed already), and the core's comparison compares each "
                         "pair of objects once; measured: values whose 46 to 91 containers each hold the one "
                         "below twice (unfolding to 2**45 .. 2**90) are taken, handed out and compared in at "
    ...
    "channel_payloads": {
        "rule": "what crosses a path (an order or cancel request, a venue report, a fill notice, an "
                ...
                "itself (str, float, int, bool, frozen plain data -- whose Fractions and Decimals are the "
                "core's own PlainFraction / PlainDecimal, round 15), never an object, a subclass instance "
