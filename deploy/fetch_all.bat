...
cd /d "%~dp0.."
rem 2026-09-21 (L-336/L-337): the log moved from logs\fetch.out.log to logs\fetch_all.out.log.
rem The old file became un-appendable ("used by another process", no visible handle) on
rem 2026-09-18 and every step here failed for three days. Leave the old file alone.
...
".venv\Scripts\python.exe" "scripts\fetch_okx.py" >> "logs\fetch_all.out.log" 2>&1
...
".venv\Scripts\python.exe" "scripts\extract_tape.py" --board-top 10 >> "logs\fetch_all.out.log" 2>&1
...
".venv\Scripts\python.exe" "scripts\record_funding_basis.py" >> "logs\fetch_all.out.log" 2>&1
...
".venv\Scripts\python.exe" "scripts\fetch_jpx_daily.py" >> "logs\fetch_all.out.log" 2>&1
".venv\Scripts\python.exe" "scripts\paper_on1.py" >> "logs\fetch_all.out.log" 2>&1
...
".venv\Scripts\python.exe" "scripts\paper_onr.py" >> "logs\fetch_all.out.log" 2>&1
rem crowd-heat gauge series (Wikipedia/GDELT/F&G) for the dashboard tile
".venv\Scripts\python.exe" "scripts\fetch_attention.py" >> "logs\fetch_all.out.log" 2>&1
rem Binance daily futures metrics (G6 features / regime) + USDJPY (yen conversion)
".venv\Scripts\python.exe" "scripts\fetch_binance_daily.py" >> "logs\fetch_all.out.log" 2>&1
...
".venv\Scripts\python.exe" "scripts\research_clock_burst.py" --status-json "data\s12_status.json" >> "logs\fetch_all.out.log" 2>&1
...
".venv\Scripts\python.exe" "scripts\retention_snapshot.py" >> "logs\fetch_all.out.log" 2>&1
...
".venv\Scripts\python.exe" "scripts\intake_ledger.py" >> "logs\fetch_all.out.log" 2>&1
...
".venv\Scripts\python.exe" "scripts\verify_snapshots.py" >> "logs\fetch_all.out.log" 2>&1
...
".venv\Scripts\python.exe" "scripts\check_data_ledger.py" --json "data\LEDGER_CHECK.json" >> "logs\fetch_all.out.log" 2>&1
