...
powershell -NoProfile -Command "if (Test-Path 'logs\fetch_all.out.log') { Select-String -Path 'logs\fetch_all.out.log' -Pattern 'extract_tape|board row|record_funding|probe_api|Traceback|Error' | Select-Object -Last 200 | ForEach-Object { $_.Line } | Set-Content 'paper_logs\fetch.out.collectors.log' }" >nul 2>&1
...
if exist data\LEDGER_CHECK.json  copy /Y data\LEDGER_CHECK.json  paper_logs\ >nul 2>&1
