...
"""Item 3 battery runner (検証・再現・出力).
    ...
    python3 tests/bt/battery/item_3/run_battery.py --list-targets

Every scene is run TWICE, each pass in a fresh process under the target's own
interpreter (a survey tool runs under its isolated venv, see i3_targets.py).
For every request the runner writes the request's files into a fresh
directory (the request's ``root``) and removes it afterwards.
...
Correctness: 正解と一致 / 対応なし / 不一致 / 結果なし (i3_judge.py).
Reproducibility: 「2 回の実行で同じ」 when both passes give the same class and
the same digest of the stable view of the observation (i3_judge.stable_view:
the repository HEAD, working-tree diff hash and raw run ids are replaced by
placeholders, since they may legitimately change between passes), 「2 回で違う
...
SCENE_TIMEOUT_S = int(os.environ.get("I3_SCENE_TIMEOUT_S", "300"))
PASS_TIMEOUT_S = int(os.environ.get("I3_PASS_TIMEOUT_S", "3600"))
...
def load_target(name: str):
    import i3_targets
    spec = i3_targets.TARGETS[name]
    path = HERE / spec["module"]
    sys.path.insert(0, str(path.parent))
    mspec = importlib.util.spec_from_file_location(f"i3_target_{name}", path)
...
def materialize(inp: dict, base: str) -> str:
    root = tempfile.mkdtemp(prefix="i3_scene_", dir=base)
    for f in inp.get("files", []):
        p = os.path.join(root, f["path"])
        os.makedirs(os.path.dirname(p), exist_ok=True)
        if "copy_from_repo" in f:
            shutil.copyfile(REPO / f["copy_from_repo"], p)
        else:
...
def run_one(target, inp, base):
    ...
    from i3_protocol import NotExpressible, Refused
    ...
    try:
        ...
        os.environ.clear()
        os.environ.update(env_before)  # an adapter's env for one request never leaks into the next
        os.chdir(cwd_before)
        shutil.rmtree(root, ignore_errors=True)


def classify(scene, target, base):
    import i3_judge as J
    ...
    cls, detail = J.compare(scene["expect"], val, scene)
    ...
    obs_rec["variant"] = vval if vkind == "ok" else {"ended": vkind}
    vcls, vdetail = J.variant_outcome(scene["variant_expect"], vkind, vval)
    return vcls, vdetail, obs_rec


def child(name: str, jsonl: str, only: list[str] | None, base: str) -> int:
    import i3_judge as J
    import i3_scenes as S
    try:
...
def interpreter(name: str) -> tuple[str, dict]:
    ...
    env.setdefault("MLFLOW_DISABLE_TELEMETRY", "true")  # survey rule §6-2: no telemetry leaves this machine
    env.setdefault("DO_NOT_TRACK", "true")
    if spec.get("venv"):
        return str(Path(i3_targets.VENV_ROOT) / spec["venv"] / "bin" / "python"), env
...
def main() -> int:
    ...
    if a.list_targets:
        import i3_targets
        for k, v in i3_targets.TARGETS.items():
