...
3. **規則 3(観点ごとにまとめ、各観点に値の場面)**: `test_battery_item3.py` が C3-1〜C3-19 のそれぞれに値の場面が 1 つ以上あることを
   機械で確かめる。→ 潰した。
4. **規則 4(動かせた道具は全部の場面に通す)**: 動かせた道具は全 52 場面に通し、渡せない場面は adapter が「何を探し、どこに無かったか」
   (道具のコードの grep = `opponents/SEARCH.tsv` と、当たりが口でない理由の行)を注記に残す。時間の打ち切りは 1 件も無い
   (1 回の実行はどれも数十秒以内。実測は `survey_results/*.tsv` の secs の列)。→ 潰した。
