...
- **他の役に消された venv**: 候補 2・3・4・10・16・18・20・34・35・37・53・65・87・105・107 の対象の venv(item_0 の下)は、この役がその対象を最後に走らせたあと(遅いもので 18:20Z 台)、18:28Z までに消えた(この役は消していない。リードの見回りの記録 commit 08aaad4「remaining item-0 venvs removed」)。survey_results の結果は今の場面集と今の共通部品で取ったもの(試験 `test_every_survey_target_ran_every_scene` が全場面の行を確かめる)だが、場面集を変えると入れ直すまで走らせ直せない。
- **C2-9 に発注の遅れだけの場面が無い**: 発注の遅れだけを持つ道具(37)は c2-9 の場面を 1 つも取れない(c2-9-order-feed は配信の遅れも要る)。発注だけの場面を足すと、上の venv の消えた対象を走らせ直せず規則 4 を破るので、この起動では足さなかった。
