...
class LuczinsritterAdapter(Adapter):
    ...
    def _units(self, sc):
        return C.unit_time(sc, [], tried='時刻の入口は get_data が返す表の添字だけで、単位を受ける引数が無い' + '(r16-1 の場面係が道具の配布物を fromtimestamp・unit=・timestamp_to_datetime・datetime64[s/ms/us]・/1000・*1000・epoch で検索した範囲。記録 survey_results/attempts/r16-1_unit_entries.txt)' + "。" + C.iso_entry_tried(self._iso, sc))
