...
class VnpyAdapter(Adapter):
    ...
    def _units(self, sc):
        return C.unit_time(sc, [], tried='時刻の変換は vnpy.alpha.to_datetime(文字列を受ける。polars が無く読み込めない)だけで、数の時刻を読む変換は無い' + '(r16-1 の場面係が道具の配布物を fromtimestamp・unit=・timestamp_to_datetime・datetime64[s/ms/us]・/1000・*1000・epoch で検索した範囲。記録 survey_results/attempts/r16-1_unit_entries.txt)' + "。" + C.iso_entry_tried(self._iso, sc))
