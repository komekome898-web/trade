...
class PineforgeAdapter(Adapter):
    ...
    def _units(self, sc):
        return C.unit_time(sc, [], tried='時刻は pf_bar_t.timestamp の Unix ミリ秒の整数のまま持ち(native_c_api.h、src/c_abi.cpp の timestamp_ms)、int ナノ秒にする変換が無い' + '(r16-1 の場面係が道具の配布物を fromtimestamp・unit=・timestamp_to_datetime・datetime64[s/ms/us]・/1000・*1000・epoch で検索した範囲。記録 survey_results/attempts/r16-1_unit_entries.txt)' + "。" + C.iso_entry_tried(self._iso, sc))
