"""Probe (round r6-1, scene keeper): Basana's own merge-by-time and same-time
...
res = run_battery.checked(B.BasanaAdapter().scene_p1_merge_by_time(sc), sc)
...
print("P1-MERGE delivered", json.dumps(out.get("sequence")), "carriers", (res.provenance or {}).get("carriers"))
...
for hand_over in permutations(KEEP):
    ...
    why = run_battery._carrier_problem([g[0] for g in got], carriers)
    orders.add(json.dumps(got))
    print("P5 hand_over", list(hand_over), "delivered", json.dumps(got), "carriers", carriers,
