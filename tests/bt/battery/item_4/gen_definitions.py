...
- **R-O1**(同じ足の中の順): 開いている建玉に、足 j で次の出口のうち 2 つ以上が起きうるときは、この順で最初の 1 つだけが建玉を閉じ、残りは起きない。
  ① 構造的な逆指値(R-W3、足 j の始値)② 足 j が保有の上限の足 b + N なら、足 j の範囲の逆指値(R-P3)、無ければ時間切れ(R-H1、足 j の始値)
  ③ 足 j − 1 の決済の合図(R-T1、足 j の始値。taker の執行)④ 逆指値(R-P3、足 j の範囲)⑤ 利確(R-P4)⑥ maker の利確(R-X1)
  ⑦ 待っている決済の指値(R-M1。maker の執行)。
  **始値の出来事(①・②の時間切れ・③)は足の範囲の出来事(④〜⑦)より先**: 足 j の始値は足 j の範囲より前に起きる(R-T1 の「始値」)。
  始値で閉じた建玉には、その足の逆指値・利確・maker の利確・指値は起きない(同じ足で建て直さない、R-T3)。②の中で逆指値が時間切れより先なのは
  R-H3(保守側)。④〜⑦の間の順は、足の範囲の中の順が分からないので決めで置いたもの: 逆指値が先なのは R-P3(保守側)、⑤ → ⑥ → ⑦ の順は
  既存の文書に無く、既存の実装の計算の順に置いた(規則の文は既存の文書を固定したもので、文書が黙る所だけ既存の計算に合わせた)。
  ①・② の時間切れ・③ の 3 つは、どれも足 j の始値の taker の決済で同じ約定になる(同じ足で建て直さない R-T3 なので、順を決めても答えは
  変わらない。この 3 組は場面で固めない)。出口の組のうち起きうる全部(両立しない組と同じ約定になる組を除く 20 組)を、場面が固める
...
- **R-M5**(取り逃しの数え方): 取り逃しに数えるのは R-M2(寿命)と R-M3(反対向きの合図による置き換え)の 2 つだけ。建玉が閉じたために捨てた
  決済の指値(R-O1 の ①〜⑥ が先に起きた)と、データの終わりに待っていた指値は数えない。
