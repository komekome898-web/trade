...
- the answers are hand-calculated from the stated rules R-* (no implementation's output is an answer; every scene has
  one answer);
- the touchstone plants exactly one defect and it breaks the scenes it says it breaks (measured on the target it wraps);
    ...
    for s in S.SCENES:
        ...
        for inp in inps:
            if inp["op"] == "bars":
                ...
                for sub in (s["judge"].values() if "engine" in s["judge"] else [s["judge"]]):
                    for k in sub:
                        if k != "invariants":
...
def _new():
    """The target the touchstone wraps (adapters/new_impl.py, the new implementation's public API).  The scene set's
    own answers never come from it; it is only the thing the touchstone's planted defect is measured on."""
    sys.path.insert(0, str(REPO / "src"))
    return _load(HERE / "adapters" / "new_impl.py", "i4_test_new_impl").TARGET
...
def test_mutant_plants_one_defect_and_breaks_its_scenes():
    ...
    for sid in ("i4-16-missed", "i4-3-e2e-maker"):
        s = S.by_id(sid)
        assert J.compare(s["expect"], new.run(s["input"]), s)[0] == "正解と一致"
...
def test_runner_writes_a_row_per_scene_with_reproducibility(tmp_path):
    ...
    ids = "i4-4-plain,i4-9-strict,i4-18-refuse"
